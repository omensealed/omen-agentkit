...
class ReleaseCandidateTests(unittest.TestCase):
    ...
    def test_local_candidate_evidence_is_specific_and_non_publishing(self) -> None:
        ...
        self.assertIn("No 0.5.0 artifact or GitHub release was published", body)
    ...
    def test_release_gate_remains_fail_closed_until_human_commit_and_tag(self) -> None:
        report = verify_release_safety(
            ...
            actual_ref=f"refs/tags/v{CANDIDATE_VERSION}",
    ...
    def test_compatibility_window_remains_binding(self) -> None:
        ...
        candidate = (ROOT / "docs" / f"RELEASE-CANDIDATE-{CANDIDATE_VERSION}.md").read_text(encoding="utf-8")
        for phrase in ("T0", "T+1", "No public Python module or CLI command is formally deprecated"):
            self.assertIn(phrase, migration)
        self.assertIn(f"Compatibility shims remain required through {CANDIDATE_VERSION}", candidate)
    ...
    def test_candidate_runner_is_packaged(self) -> None:
        ...
        self.assertIn(f"include docs/RELEASE-CANDIDATE-{CANDIDATE_VERSION}.md", manifest)
        self.assertNotIn(f"exclude docs/RELEASE-CANDIDATE-{CANDIDATE_VERSION}.md", manifest)
        self.assertNotIn("include docs/IMPLEMENTATION-NOTES.md", manifest)
        self.assertNotIn("include docs/PROGRESS.md", manifest)
        self.assertIn("include .github/ISSUE_TEMPLATE/migration-burn-in.yml", manifest)

    def test_private_maintainer_ledgers_are_not_public_checkout_dependencies(self) -> None:
        ignored = (ROOT / ".gitignore").read_text(encoding="utf-8").splitlines()
        self.assertIn("docs/IMPLEMENTATION-NOTES.md", ignored)
        self.assertIn("docs/PROGRESS.md", ignored)
        for public_doc in ("AGENT-GUIDE.md", "MAINTAINER-GUIDE.md"):
            body = (ROOT / "docs" / public_doc).read_text(encoding="utf-8")
            self.assertNotRegex(body, r"\[[^]]+\]\((?:IMPLEMENTATION-NOTES|PROGRESS)\.md\)")
