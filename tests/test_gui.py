...
class GuiTests(unittest.TestCase):
    ...
    def test_gui_app_imports_without_pywebview(self) -> None:
        ...
        self.assertIn('id="compose-task" disabled', html)
        self.assertIn('Complete each required task answer', html)
        ...
        self.assertIn('function validatedTaskPayload()', script)
        self.assertIn('const payload = validatedTaskPayload();', script)
        self.assertIn('control.required = question.required;', script)
        self.assertIn('Answer required:', script)
        self.assertIn('missing.setAttribute("aria-invalid", "true")', script)
        draft_resume = script.split("function applyDraftTask", 1)[1].split("async function refreshDrafts", 1)[0]
        self.assertIn('updateComposeAvailability();', draft_resume)
