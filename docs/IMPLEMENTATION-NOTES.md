# Implementation notes

This is the starter kit’s durable engineering journal. Add a dated entry after every meaningful phase. Record files changed, design decisions, exact commands, results, implications, unresolved issues, and the next concrete step.

## 2026-06-23T00:00:00Z — initial implementation

- Objective/phase: Build the first complete beginner-oriented CLI agent project wizard for CachyOS.
- Files/subsystems changed: Python package, templates, examples, tests, install helpers, generated-project scripts, security documentation, and source documentation.
- Behavior/design decisions: Use Python standard library only; delegate account authorization to official clients; preserve existing files through proposals/backups; make package installation explicit; require generated implementation notes.
- Commands and tests run: Python unit/integration suite, shell syntax checks, fresh-project generation/validation, generated checks, and isolated install/uninstall smoke test.
- Results: Initial release workflow completed.
- Security/data/performance/UX implications: No credential storage; no automatic privileged installation; no silent existing-file replacement; beginner prompts include security and test planning.
- Unresolved problems: Live account authorization remains an external acceptance step because automated tests intentionally cannot access user credentials.
- Exact next step: Refine the package as real project usage reveals workflow gaps.

## 2026-06-24T00:00:00Z — Codex-only refactor

- Objective/phase: Make the starter kit one-to-one with OpenAI Codex CLI and remove every alternate-agent workflow.
- Files/subsystems changed: Agent adapter registry, CLI commands, wizard, project model validation, generator, templates, examples, tests, root documentation, and all maintainer docs.
- Behavior/design decisions: Keep `primary_agent` as a fixed `codex` metadata invariant; expose one OAuth path, one advisor, one launch path, and one generated `.codex` configuration; eliminate agent-selection questions and compatibility files.
- Commands and tests run: `python3 -m compileall -q agent_starter tests starter.py`; `python3 -m unittest discover -s tests -v`; full `./scripts/check.sh`; clean-extraction verification before packaging.
- Results: All 34 unit/integration tests passed; source and clean-extraction `./scripts/check.sh` runs passed; new Python/SQLite and existing PHP/MariaDB generated workspaces validated; existing source preservation and Codex-only output checks passed.
- Security/data/performance/UX implications: Smaller attack and maintenance surface; fewer confusing choices for beginners; credentials remain entirely controlled by the official Codex CLI; existing safe-write and explicit-install boundaries remain intact.
- Unresolved problems: Live ChatGPT OAuth cannot be exercised in automated packaging tests and must be completed by the end user through `codex login`.
- Exact next step: Distribute the version 0.2.0 archive and checksum, then collect feedback from the first real CachyOS/Codex project run.

## 2026-06-24T15:05:47Z — maintainer orientation baseline

- Objective/phase: Establish a verified baseline before feature work by reading the maintainer contract, documentation, implementation, tests, scripts, examples, and generated output.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only; runtime code, templates, tests, examples, and shell scripts were inspected but not modified.
- Behavior/design decisions: Confirmed the current architecture remains Codex-only, Python-standard-library at runtime, review-first for CachyOS package installation, and protective of existing project files through proposals/backups instead of silent replacement.
- Commands and tests run: `git status --short` and `git log -5 --oneline` failed because this extracted directory is not a Git repository; `./scripts/check.sh` passed before this note; generated and inspected `/tmp/tmp.leP7HW0477/new-python-sqlite` from `examples/cli-python-sqlite-codex.json`; generated and inspected `/tmp/tmp.leP7HW0477/existing-php-mariadb` from `examples/existing-php-mariadb-renovation.json` with a preexisting `README.md`, confirming a proposal was created and the original file remained untouched.
- Results: Source checks passed before the note with 34 unit/integration tests passing, shell syntax validation passing, generation smoke passing, and isolated install/uninstall smoke passing; generated Python/SQLite and existing PHP/MariaDB workspaces validated successfully.
- Security/data/performance/UX implications: No Codex login, OpenAI network call, package installation, `sudo`, GitHub publication, remote push, deployment, production-data access, credential-store inspection, or AI-suggested command execution was performed.
- Unresolved problems: The workspace has no Git metadata, so branch, dirty state, and recent commit history cannot be verified from this directory; live Codex OAuth remains an external acceptance item.
- Exact next step: Rerun `./scripts/check.sh` after this documentation-only note, then begin feature planning from the verified baseline.

## 2026-06-24T15:12:27Z — Codex continuation prompt generator

- Objective/phase: Add a user-facing way to generate a well-formed copy/paste Codex prompt for the next project phase or a requested feature without launching Codex automatically.
- Files/subsystems changed: `agent_starter/cli.py`, `tests/test_cli.py`, `README.md`, `docs/USER-GUIDE.md`, `docs/AGENT-INTEGRATION.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added `agent-starter prompt [project] --request "..."` with optional `--phase`, `--output`, and `--force`; the command loads generated metadata, rejects credential-like request text, prints to stdout by default, and refuses to overwrite output files unless `--force` is supplied.
- Commands and tests run: `python3 -m unittest tests.test_cli -v` passed with 7 tests; `./agent-starter prompt /tmp/tmp.leP7HW0477/new-python-sqlite --request "Add CSV import"` produced a focused continuation prompt; `./scripts/check.sh` passed with 36 unit/integration tests plus syntax checks, CLI smoke, generation smoke, generated-project check, and install/uninstall smoke.
- Results: Continuation prompt generation is covered by regression tests and documented for users; full source verification passed.
- Security/data/performance/UX implications: The prompt generator uses project metadata and document paths rather than copying raw project docs into the prompt; it reinforces sandboxing, approval prompts, no credential access, no automatic command execution, no `sudo`, no remote pushes, and no destructive/publish/deploy actions without explicit approval.
- Unresolved problems: The command does not yet offer an interactive questionnaire for shaping ambiguous feature requests; users provide the request string directly.
- Exact next step: Rerun `./scripts/check.sh` after this note and then consider whether generated projects should also include a local helper script for continuation prompts.

## 2026-06-24T15:21:03Z — Ollama local-model readiness gate

- Objective/phase: Add a guarded way for users to assess whether installed Ollama models are plausible local handoff targets before generating a continuation prompt.
- Files/subsystems changed: `AGENTS.md`, `agent_starter/cli.py`, `tests/test_cli.py`, `README.md`, `docs/USER-GUIDE.md`, `docs/AGENT-INTEGRATION.md`, `docs/ARCHITECTURE.md`, `docs/SECURITY-MODEL.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added `agent-starter ollama-check [project]` with optional `--model`, `--request`, `--output`, `--force`, and `--override`; the command inspects `ollama list` plus `ollama show <model> --json`, extracts confirmed context length when available, scores model-name coding suitability, and refuses borderline/inadvisable handoff prompt generation without explicit override.
- Commands and tests run: OpenAI Codex manual helper was attempted for current product/model context but failed with DNS resolution for `developers.openai.com`; `python3 -m unittest tests.test_cli -v` passed with 10 tests; `./scripts/check.sh` passed with 39 unit/integration tests plus syntax checks, CLI smoke, generation smoke, generated-project check, and install/uninstall smoke.
- Results: Local-model handoff is assessment-only and covered by mocked Ollama regression tests for suitable, blocked, and override paths.
- Security/data/performance/UX implications: The feature does not install Ollama, pull models, send repository content to Ollama, execute local-model output, rewrite `.codex/config.toml`, alter `primary_agent`, or add an alternate generated launcher/auth path; warnings remain embedded in override-generated handoff prompts.
- Unresolved problems: Context and coding suitability are based on local Ollama metadata and model-name heuristics, not a live benchmark; current OpenAI model specifics were not verified because official docs were unreachable from this environment.
- Exact next step: Rerun `./scripts/check.sh` after this note, then consider whether an optional generated-project helper should wrap `agent-starter ollama-check` when the starter is installed.

## 2026-06-24T15:23:32Z — Codex manual baseline verification

- Objective/phase: Use the freshly available Codex manual cache to verify the local-model handoff baseline.
- Files/subsystems changed: `agent_starter/cli.py`, `docs/USER-GUIDE.md`, `docs/AGENT-INTEGRATION.md`, `docs/ARCHITECTURE.md`, and this implementation note.
- Behavior/design decisions: Added `gpt-5.5` as the explicit Codex reference baseline in `ollama-check` output and local-model handoff prompts because the Codex manual lists it as the recommended model for complex coding work; kept Ollama readiness as a conservative metadata heuristic rather than a claim of local parity.
- Commands and tests run: Read `/tmp/openai-docs-cache/codex-manual.outline.md`; read manual sections for config basics, model selection, sandboxing, and `AGENTS.md` discovery.
- Results: Confirmed project `.codex/config.toml` scoping, `workspace-write` plus `on-request` as a lower-risk local automation posture, `AGENTS.md` instruction discovery, and `gpt-5.5` as the current recommended Codex model.
- Security/data/performance/UX implications: The starter still does not rewrite Codex configuration, add an alternate launcher, or send project content to Ollama; users get a clearer comparison baseline before overriding local-model warnings.
- Unresolved problems: Ollama model capability still cannot be guaranteed without task-specific evaluation; context length and model-name signals remain only a readiness screen.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T17:28:15Z — AGPL generated license option

- Objective/phase: Add `AGPL-3.0-or-later` as a built-in license choice for generated projects.
- Files/subsystems changed: `agent_starter/wizard.py`, `agent_starter/generator.py`, `agent_starter/templates.py`, `tests/test_templates.py`, `tests/test_generator.py`, `docs/ANSWER-FILE-REFERENCE.md`, `docs/USER-GUIDE.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: The wizard now offers MIT, AGPL-3.0-or-later, and Undecided. The generator writes a concise AGPL notice with `SPDX-License-Identifier: AGPL-3.0-or-later` and the official GNU AGPL URL instead of embedding a large license body template.
- Commands and tests run: `python3 -m unittest tests.test_templates tests.test_generator -v` passed with 17 tests.
- Results: AGPL project license generation is covered by template and generator regression tests.
- Security/data/performance/UX implications: No runtime dependency or external command was added; existing MIT and Undecided behavior remains intact.
- Unresolved problems: The generated AGPL file is a notice and URL, not the full AGPL text.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T17:29:39Z — AGPL generated-output inspection

- Objective/phase: Inspect a freshly generated AGPL project after adding the license option.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: Used the existing Python/SQLite example as the base and changed only destination, project name, Git initialization, and `license_name` to `AGPL-3.0-or-later`.
- Commands and tests run: Generated `/tmp/tmp.8Ea4HeRbSK/agpl-project`; inspected `LICENSE`; checked metadata for `license_name` and `primary_agent`; ran `bash -n` on generated scripts; ran `./agent-starter validate /tmp/tmp.8Ea4HeRbSK/agpl-project --verbose`; ran `/tmp/tmp.8Ea4HeRbSK/agpl-project/scripts/check.sh`.
- Results: Generated workspace created 43 files, validation passed, generated shell syntax passed, and generated `scripts/check.sh` passed. `LICENSE` contained the AGPL title, SPDX identifier, and GNU license URL. Metadata preserved `license_name: AGPL-3.0-or-later` and `primary_agent: codex`.
- Security/data/performance/UX implications: No network, Codex login, package installation, `sudo`, GitHub action, or external model execution was performed.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T17:31:33Z — full AGPL license text

- Objective/phase: Resolve the AGPL license-quality gap by generating the full AGPL-3.0-or-later license text instead of only a short notice.
- Files/subsystems changed: `agent_starter/templates.py`, `tests/test_templates.py`, and this implementation note.
- Behavior/design decisions: Embedded the local SPDX `AGPL-3.0-or-later.txt` license text from `/usr/share/licenses/spdx/AGPL-3.0-or-later.txt`, preceded by `SPDX-License-Identifier: AGPL-3.0-or-later`. Removed the unused short-notice helper left from the prior implementation.
- Commands and tests run: Inspected the local SPDX license file; verified the template returns 237 lines and includes section 13.
- Results: Generated AGPL projects will now include the full AGPL text plus SPDX identifier in `LICENSE`.
- Security/data/performance/UX implications: No network lookup, runtime dependency, or generated command behavior changed.
- Unresolved problems: Need focused tests, generated-output inspection, and full source check after this note.
- Exact next step: Run focused template/generator tests and inspect a fresh AGPL output.

## 2026-06-24T17:33:50Z — full AGPL verification

- Objective/phase: Verify the full AGPL license text change end to end.
- Files/subsystems changed: `docs/ANSWER-FILE-REFERENCE.md` and this implementation note.
- Behavior/design decisions: Preserved the short SPDX notice helper for Apache, BSD, GPL, and MPL license outputs while making AGPL the only generated license that embeds the full text.
- Commands and tests run: `python3 -m unittest tests.test_templates tests.test_generator -v` passed with 18 tests; generated `/tmp/tmp.2MvKE096r7/agpl-project`; ran `./agent-starter validate /tmp/tmp.2MvKE096r7/agpl-project`; ran `/tmp/tmp.2MvKE096r7/agpl-project/scripts/check.sh`; inspected generated `LICENSE` with `wc -l` and `rg`; ran `./scripts/check.sh`, which passed with 42 tests plus syntax checks, CLI smoke, generation smoke, generated-project check, and install/uninstall smoke.
- Results: The generated AGPL `LICENSE` is 237 lines and includes `SPDX-License-Identifier: AGPL-3.0-or-later`, `GNU AFFERO GENERAL PUBLIC LICENSE`, and section 13. Full source verification passed.
- Security/data/performance/UX implications: No network, package installation, Codex login, `sudo`, GitHub action, or external model execution was performed.
- Unresolved problems: None for this AGPL license substep.
- Exact next step: Rerun `./scripts/check.sh` after this note.

## 2026-06-24T17:35:26Z — license option matrix inspection

- Objective/phase: Verify every built-in license option through actual project generation and validation.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: Confirmed the interactive wizard already exposes the same built-in license choices supported by the generator and answer-file docs: MIT, Apache-2.0, BSD-3-Clause, GPL-3.0-or-later, AGPL-3.0-or-later, MPL-2.0, and Undecided.
- Commands and tests run: Generated and validated temporary projects under `/tmp/tmp.j0LRs4yT7s` for all seven license modes; inspected `LICENSE` markers for each generated license; confirmed Undecided generated no `LICENSE`; ran `/tmp/tmp.j0LRs4yT7s/agpl-3-0-or-later/scripts/check.sh`.
- Results: All license-mode workspaces generated and validated successfully. AGPL emitted a 237-line full license, MIT emitted 21 lines, Apache/BSD/GPL/MPL emitted 8-line SPDX notices, and Undecided omitted `LICENSE` as expected. Representative generated check passed.
- Security/data/performance/UX implications: No network, package installation, Codex login, `sudo`, GitHub action, or external model execution was performed.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T17:38:32Z — beginner license guidance

- Objective/phase: Reduce beginner uncertainty during license selection.
- Files/subsystems changed: `agent_starter/wizard.py`, `tests/test_wizard.py`, `docs/USER-GUIDE.md`, and this implementation note.
- Behavior/design decisions: Added a short license quick guide immediately before the wizard license choice, explaining permissive licenses, copyleft licenses, AGPL network-service source-sharing obligations, MPL file-level copyleft, and Undecided as the safe choice when unsure.
- Commands and tests run: `python3 -m unittest tests.test_wizard -v` passed with 4 tests.
- Results: Interactive users now see plain-language license guidance before choosing from the built-in license options.
- Security/data/performance/UX implications: No generated file contract, runtime dependency, external command, or licensing behavior changed; the update is user guidance only.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T18:02:14Z — GitHub AI-local artifact hygiene

- Objective/phase: Keep GitHub uploads focused on core project source, durable docs, and scripts by excluding local AI/runtime artifacts from generated Git tracking.
- Files/subsystems changed: `agent_starter/toolchains.py`, `tests/test_toolchains.py`, `tests/test_generator.py`, `README.md`, `docs/TEMPLATE-CATALOG.md`, `docs/USER-GUIDE.md`, and this implementation note.
- Behavior/design decisions: Expanded generated `.gitignore` to exclude local Codex logs/session files, starter runtime state, proposal and backup directories, saved continuation prompts, and local-model handoff drafts. Durable project memory such as `AGENTS.md`, numbered docs, scripts, `.codex/config.toml`, and `.agent-starter/project.json` remains trackable because it is part of the generated workspace contract.
- Commands and tests run: `python3 -m unittest tests.test_toolchains tests.test_generator -v` passed with 13 tests; generated `/tmp/tmp.EVq58TZuTA/project`; used `git check-ignore` to verify local AI artifacts were ignored and core files were not ignored.
- Results: Generated Git-enabled projects now ignore AI-local artifacts while preserving trackability for the core project contract.
- Security/data/performance/UX implications: Reduces accidental upload of prompts, local handoff drafts, Codex logs, session JSONL, runtime state, backups, and conflict proposals. No GitHub remote, push, network call, Codex login, package installation, or `sudo` was used.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T18:10:09Z — local-first GitHub defaults

- Objective/phase: Reduce beginner GitHub/CI noise by deferring GitHub Actions and remote setup until local checks prove the project is worth publishing or backing up remotely.
- Files/subsystems changed: `agent_starter/wizard.py`, `agent_starter/models.py`, `agent_starter/cli.py`, `examples/*.json`, `scripts/smoke-test.sh`, `tests/test_wizard.py`, `README.md`, `docs/USER-GUIDE.md`, `docs/SECURITY-MODEL.md`, `docs/ANSWER-FILE-REFERENCE.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Local Git remains default-on, but interactive and example defaults now keep `github_actions` false and `github_remote` none. The wizard prints a local-first recommendation before asking about GitHub Actions. Explicit `github_actions: true` answers still generate `.github/workflows/ci.yml`.
- Commands and tests run: `python3 -m unittest tests.test_wizard tests.test_cli tests.test_release -v` passed with 15 tests; `./scripts/smoke-test.sh` passed and confirmed default examples do not generate GitHub Actions; generated an explicit-CI project with `github_actions: true`, confirmed `.github/workflows/ci.yml` exists and calls `./scripts/check.sh`, and validated the workspace.
- Results: New users get a calmer local-first path, while CI remains available by explicit opt-in.
- Security/data/performance/UX implications: Reduces premature GitHub Actions noise, token use, and remote-platform distraction. No GitHub remote, push, network call, Codex login, package installation, or `sudo` was used.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T22:21:45Z — Phase A first-run clarity

- Objective/phase: Fill the first beginner-usability gap in the sandbox manager maturity plan by generating an explicit post-generation next-steps guide.
- Files/subsystems changed: `agent_starter/generator.py`, `agent_starter/templates.py`, `agent_starter/cli.py`, `tests/test_generator.py`, `tests/test_templates.py`, `tests/test_cli.py`, `scripts/smoke-test.sh`, `README.md`, `docs/USER-GUIDE.md`, `docs/TEMPLATE-CATALOG.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added required root `NEXT_STEPS.md` to generated projects, made CLI completion output point at it, and changed placeholder `build`, `lint`, and `test` scripts to state clearly that no real app harness exists yet. `NEXT_STEPS.md` keeps the first session local-first, explains Codex setup and launch, preserves the GitHub pause, and points to continuation prompt and Ollama assessment helpers without adding alternate-agent configuration.
- Commands and tests run: `python3 -m unittest tests.test_templates tests.test_generator tests.test_cli -v` passed with 30 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; `./scripts/smoke-test.sh` passed; generated `/tmp/tmp.ZvuycuFJdU/python-sqlite` from `examples/cli-python-sqlite-codex.json`; generated `/tmp/tmp.ZvuycuFJdU/existing-renovation` from `examples/existing-php-mariadb-renovation.json` after seeding an existing `index.php`; validated both workspaces; ran Bash syntax checks across both generated script sets; inspected `NEXT_STEPS.md`, `AGENTS.md`, and `FIRST_PROMPT.md` for Codex-specific and implementation-note contracts.
- Results: Focused tests, smoke generation, representative generated workspace validation, and generated shell syntax all passed. Generated `NEXT_STEPS.md` is required by validation and uses new-project placeholder wording only for new projects, with separate existing-project guidance for renovation workspaces.
- Security/data/performance/UX implications: Improves beginner guidance without running Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, or external model execution. Existing safe-write, conflict proposal, backup, symlink, traversal, and Codex-only constraints remain unchanged.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T22:24:00Z — Phase A verification

- Objective/phase: Record the final verification result for the Phase A first-run clarity change.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 44 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, or external model execution.
- Unresolved problems: None for this Phase A substep.
- Exact next step: Review the diff, then move to the next maturity-plan phase when ready.

## 2026-06-24T22:32:37Z — Phase B workspace status command

- Objective/phase: Add the first Phase B readiness feature so beginners can ask the starter what state a generated workspace is in and what to do next.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/templates.py`, `tests/test_cli.py`, `tests/test_templates.py`, `scripts/smoke-test.sh`, `README.md`, `docs/USER-GUIDE.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added read-only `agent-starter status /path/to/project`. The command loads non-secret project metadata, validates required generated files, checks Codex install/auth status through the official CLI status boundary, summarizes local Git and GitHub Actions state, verifies key AI-local `.gitignore` protections, and prints one next recommended action. Generated `NEXT_STEPS.md` now includes `agent-starter status .`.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_templates tests.test_generator -v` passed with 32 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; `./scripts/smoke-test.sh` passed and exercised `agent-starter status` on a generated workspace; `python3 -m unittest tests.test_cli tests.test_templates -v` passed with 23 tests after the smoke-test update.
- Results: The new status command reports healthy generated workspaces, missing metadata failures, Codex readiness, Git/GitHub posture, ignored AI-local artifact posture, and next action guidance without mutating the project.
- Security/data/performance/UX implications: No credential files are inspected, no OAuth token is read, no login is started, no package installation or `sudo` is run, and no GitHub remote/push/deploy action is performed. The command may invoke `codex --version` and `codex login status` only when the Codex executable exists.
- Unresolved problems: The second Phase B feature, `agent-starter github-ready /path/to/project`, remains to be implemented.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T22:33:02Z — Phase B status verification

- Objective/phase: Record final verification for the `agent-starter status` readiness command.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 46 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test including `agent-starter status`, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, or external model execution.
- Unresolved problems: `agent-starter github-ready /path/to/project` remains the next Phase B gap.
- Exact next step: Implement the GitHub readiness gate.

## 2026-06-24T22:35:58Z — Phase B GitHub readiness gate

- Objective/phase: Complete Phase B by adding a local-first readiness gate before users create GitHub remotes, enable CI, or push.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/templates.py`, `tests/test_cli.py`, `tests/test_templates.py`, `scripts/smoke-test.sh`, `README.md`, `docs/USER-GUIDE.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added `agent-starter github-ready /path/to/project`. The command loads generated metadata, validates required files, runs `./scripts/check.sh` unless `--skip-check` is supplied, checks that local Git exists and is clean, verifies AI-local artifact ignore patterns, and inspects the GitHub Actions workflow when present. It reports a recommendation but never creates a repository, enables CI, pushes, installs packages, deploys, or contacts GitHub.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_templates -v` passed with 25 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; generated `/tmp/tmp.Ai4FjEXQvb/project` from `examples/cli-python-sqlite-codex.json`; ran `agent-starter github-ready` before committing and confirmed it blocked on uncommitted Git state; made a local-only baseline commit in that temporary project; reran `agent-starter github-ready` and confirmed it passed with GitHub Actions still optional/deferred.
- Results: The readiness gate blocks premature GitHub work when local state is dirty or incomplete and allows a clean local baseline without requiring GitHub Actions.
- Security/data/performance/UX implications: The command runs only local validation and the project-local check script. It does not read credentials, start Codex or GitHub login, call OpenAI/GitHub, run `sudo`, install packages, create remotes, push, or deploy. Users can pass `--skip-check` for static-only inspection.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T22:36:25Z — Phase B GitHub readiness verification

- Objective/phase: Record final verification for the completed Phase B GitHub readiness gate.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 48 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test with `status` and generated `github-ready` guidance, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, or external model execution.
- Unresolved problems: None for Phase B.
- Exact next step: Move to Phase C guided continuation prompts when ready.

## 2026-06-24T22:38:32Z — Phase C interactive continuation prompts

- Objective/phase: Start Phase C by giving beginners a guided way to generate Codex continuation prompts without hand-writing the whole request.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/templates.py`, `tests/test_cli.py`, `tests/test_templates.py`, `README.md`, `docs/USER-GUIDE.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added `agent-starter prompt --interactive`. The guided mode asks for task type, requested outcome, what changed since the last session, likely affected surfaces, risk/concerns, expected verification, and phase label. It folds those answers into the existing Codex-safe continuation prompt rather than adding a new agent path. Existing `--request` behavior remains unchanged for scripts and answerable one-liners.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_templates -v` passed with 27 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; `./scripts/smoke-test.sh` passed.
- Results: Interactive prompt generation produces structured task context, rejects credential-like entries before they enter the generated prompt, and generated `NEXT_STEPS.md` now advertises the guided prompt path.
- Security/data/performance/UX implications: The command is local text generation only. It does not launch Codex, read credentials, run OpenAI requests, install packages, use `sudo`, create GitHub resources, push, deploy, or execute generated commands.
- Unresolved problems: Phase C prompt templates for bug fix, feature, cleanup, docs, test-baseline, and release-prep work remain to be implemented.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T22:38:54Z — Phase C interactive prompt verification

- Objective/phase: Record final verification for the guided continuation prompt mode.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 50 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, or external model execution.
- Unresolved problems: Prompt templates for named task categories remain the next Phase C gap.
- Exact next step: Add prompt templates for bug fix, feature, cleanup, docs, test-baseline, and release-prep work.

## 2026-06-24T22:41:01Z — Phase C named prompt templates

- Objective/phase: Complete Phase C by adding named Codex continuation prompt templates for common beginner workflows.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/templates.py`, `tests/test_cli.py`, `tests/test_templates.py`, `README.md`, `docs/USER-GUIDE.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added `agent-starter prompt --template` with `feature`, `bug`, `cleanup`, `docs`, `test-baseline`, and `release-prep` choices. Templates add static guidance sections to the existing Codex-safe continuation prompt; they do not execute commands or change agent configuration. Interactive prompt generation now maps its task-type answer to the same template set.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_templates -v` passed with 28 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; `./scripts/smoke-test.sh` passed.
- Results: Named templates render the expected task-specific sections, interactive bug prompts include the bug-fix template, and generated `NEXT_STEPS.md` documents the available template names.
- Security/data/performance/UX implications: This is local prompt text generation only. It does not run Codex, read credentials, make OpenAI or GitHub requests, install packages, use `sudo`, push, deploy, or execute generated commands.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T22:41:24Z — Phase C template verification

- Objective/phase: Record final verification for named prompt templates and Phase C completion.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 51 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, or external model execution.
- Unresolved problems: None for Phase C.
- Exact next step: Move to Phase D local backup/mirror planning when ready.

## 2026-06-24T23:43:53Z — Phase D rsync mirror planning

- Objective/phase: Complete Phase D by adding review-first local/SSH source mirror support.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/generator.py`, `agent_starter/templates.py`, `tests/test_cli.py`, `tests/test_generator.py`, `tests/test_templates.py`, `scripts/smoke-test.sh`, `README.md`, `docs/USER-GUIDE.md`, `docs/TEMPLATE-CATALOG.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added generated `.agent-starter/rsync-excludes` and `agent-starter rsync-plan PROJECT TARGET`. The command prints the exact `rsync` argv by default and only executes with `--run`. It refuses local targets inside the project root, supports local paths and SSH-style rsync targets, and has explicit `--delete` for target-side deletion. The exclude file mirrors source and durable docs while skipping `.git/`, credentials, local databases, caches, Codex session/log files, prompt drafts, starter runtime state, proposals, and backups.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_generator tests.test_templates -v` passed with 40 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; `./scripts/smoke-test.sh` passed and exercised plan-only `rsync-plan` on a generated project.
- Results: Generated projects now include 44 files, validation requires `.agent-starter/rsync-excludes`, rsync plans are review-only by default, and explicit `--run` is covered with mocked subprocess execution.
- Security/data/performance/UX implications: The default command does not copy files. Execution requires `--run`; deletion requires `--delete`. No Codex login, OpenAI request, package installation, `sudo`, GitHub creation, remote push, deploy, or credential inspection was performed.
- Unresolved problems: Full source check still needs to run after this note.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T23:44:18Z — Phase D rsync verification

- Objective/phase: Record final verification for Phase D local mirror planning.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 54 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test with generated `.agent-starter/rsync-excludes` and plan-only `rsync-plan`, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, deployment, or real rsync execution.
- Unresolved problems: None for Phase D.
- Exact next step: Reassess the project roadmap and decide whether to cut a release or add another maturity phase.

## 2026-06-24T23:45:22Z — roadmap reassessment after Phase D

- Objective/phase: Reassess the completed sandbox manager maturity plan and define the next concrete phase.
- Files/subsystems changed: `docs/PROGRESS.md` and this implementation note.
- Behavior/design decisions: All Phase A-D maturity-plan items are complete, so the next responsible step is release readiness rather than adding another feature. Added Phase E with gates for version decision, representative generated-output inspection, source-tree full check, clean archive extraction verification, and archive/checksum handoff after approval.
- Commands and tests run: Pending full source check after this note.
- Results: Phase E is now the active next plan; no runtime behavior changed.
- Security/data/performance/UX implications: Documentation-only roadmap update. No Codex login, OpenAI request, package installation, `sudo`, GitHub creation, remote push, deployment, archive publication, or credential inspection was performed.
- Unresolved problems: Release version, archive build, clean extraction verification, and checksum publication remain pending.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T23:45:44Z — Phase E planning verification

- Objective/phase: Verify the documentation-only Phase E release-readiness plan.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 54 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test with 44 generated files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, deployment, archive publication, or real rsync execution.
- Unresolved problems: Phase E release version decision and clean archive verification remain pending.
- Exact next step: Decide the next release version and update version files together if release approval is granted.

## 2026-06-24T23:46:42Z — Phase E version decision

- Objective/phase: Complete the first Phase E gate by selecting and applying the next release version.
- Files/subsystems changed: `VERSION`, `agent_starter/__init__.py`, `agent_starter/models.py`, `pyproject.toml`, `tests/test_cli.py`, `CHANGELOG.md`, `docs/PROGRESS.md`, and this implementation note.
- Behavior/design decisions: Selected `0.3.0` because the changes since `0.2.0` add several user-facing commands and generated workspace contracts: next-steps guidance, status/readiness gates, guided prompt generation, prompt templates, local-model assessment, license additions, local-first GitHub defaults, AI-local artifact hygiene, and rsync mirror planning. Moved accumulated changelog entries from Unreleased into `0.3.0 — 2026-06-24`.
- Commands and tests run: `python3 -m unittest tests.test_release tests.test_cli -v` passed with 21 tests; `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed; `./agent-starter --version` printed `agent-starter 0.3.0`.
- Results: Version sources are synchronized at `0.3.0`, and the release consistency test passes.
- Security/data/performance/UX implications: No Codex login, OpenAI request, package installation, `sudo`, GitHub creation, remote push, deployment, archive build, archive publication, or credential inspection was performed.
- Unresolved problems: Representative generated-output inspection, full source check after this note, clean archive extraction verification, and checksum handoff remain pending.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T23:47:05Z — Phase E version verification

- Objective/phase: Record full source verification after the `0.3.0` version update.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 54 unit/integration tests, shell syntax checks, source-tree CLI version check showing `agent-starter 0.3.0`, generation smoke test with 44 generated files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, deployment, archive publication, or real rsync execution.
- Unresolved problems: Representative generated-output inspection, clean archive extraction verification, and checksum handoff remain pending.
- Exact next step: Inspect one new Python/SQLite generated project and one existing-project renovation after the 0.3.0 version update.

## 2026-06-24T23:49:26Z — Phase E generated-output inspection

- Objective/phase: Complete the representative generated-output inspection gate for the 0.3.0 release.
- Files/subsystems changed: `agent_starter/toolchains.py`, `tests/test_toolchains.py`, `docs/PROGRESS.md`, and this implementation note.
- Behavior/design decisions: Generated and inspected `/tmp/tmp.hBO3KlRedo/new-python-sqlite-final` from `examples/cli-python-sqlite-codex.json` and `/tmp/tmp.hBO3KlRedo/existing-renovation-final` from `examples/existing-php-mariadb-renovation.json` with a preexisting `README.md`. During inspection, the existing-project check exposed that JavaScript and Composer defaults could fail when `package.json` or `composer.json` did not exist. Fixed those generated defaults so manifest-based npm/composer commands skip with clear messages during discovery instead of failing the whole check.
- Commands and tests run: Generated both representative projects; validated both with `agent-starter validate --verbose`; ran `bash -n` over both generated script sets; ran both generated `scripts/check.sh`; ran `agent-starter status` on both projects; ran plan-only `agent-starter rsync-plan` on both projects; confirmed the existing README remained untouched and a proposal was created; ran `python3 -m unittest tests.test_toolchains tests.test_generator -v`, which passed with 14 tests.
- Results: The new Python/SQLite project generated 44 files, validated, and passed generated checks with placeholder messages. The existing PHP/MariaDB renovation preserved the preexisting README, created one proposal, validated with the expected proposal warning, and passed generated checks after manifest guards were added. Both projects included `NEXT_STEPS.md`, `.agent-starter/rsync-excludes`, Codex-only `AGENTS.md`, `docs/11-IMPLEMENTATION-NOTES.md`, and 0.3.0 implementation-note metadata.
- Security/data/performance/UX implications: Existing-project users no longer get a failing check merely because a selected stack has no manifest yet. No Codex login flow was started, no OpenAI request was made, no package installation or `sudo` was run, no GitHub repository was created, no remote push or deployment happened, and rsync remained plan-only.
- Unresolved problems: Full source check after this fix, clean archive extraction verification, and checksum handoff remain pending.
- Exact next step: Run `./scripts/check.sh`.

## 2026-06-24T23:49:54Z — Phase E generated-output verification

- Objective/phase: Record full source verification after the representative generated-output inspection and manifest-guard fix.
- Files/subsystems changed: `docs/IMPLEMENTATION-NOTES.md` only.
- Behavior/design decisions: No behavior changed in this follow-up; it records the completed verification gate.
- Commands and tests run: `./scripts/check.sh`.
- Results: Passed. The check ran Python syntax compilation, 55 unit/integration tests, shell syntax checks, source-tree CLI version check showing `agent-starter 0.3.0`, generation smoke test with 44 generated files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Verification did not run Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, deployment, archive publication, or real rsync execution.
- Unresolved problems: Clean archive extraction verification and checksum handoff remain pending.
- Exact next step: Build a clean 0.3.0 archive for extraction verification without caches, local Git metadata, generated workspaces, or credentials.

## 2026-06-24T23:59:58Z — Phase E clean archive verification

- Objective/phase: Build and verify a clean 0.3.0 release archive.
- Files/subsystems changed: `docs/PROGRESS.md` and this implementation note. Release artifacts were written under `/tmp`.
- Behavior/design decisions: Built `/tmp/cli-ai-agent-starter-kit-codex-0.3.0.tar.gz` with local `.git`, `.agents`, `.codex`, Python caches, and `.pyc` files excluded. Extracted it to `/tmp/tmp.FIw04Om7rQ/cli-ai-agent-starter-kit-codex-0.3.0` and verified the extracted CLI reports `agent-starter 0.3.0`.
- Commands and tests run: Listed archive contents; checked the tar listing for excluded cache/local metadata names; extracted the archive; ran `./scripts/check.sh` from the extracted archive; generated `/tmp/cli-ai-agent-starter-kit-codex-0.3.0.tar.gz.sha256`.
- Results: Extracted archive `./scripts/check.sh` passed with 55 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test with 44 generated files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. The final checksum is recorded in the sidecar file rather than embedded here so the archive can include these notes without a self-referential checksum.
- Security/data/performance/UX implications: No Codex login, OpenAI requests, package installation, `sudo`, GitHub creation, remote push, deployment, archive publication, or real rsync execution was performed. The archive excludes local agent metadata and cache files.
- Unresolved problems: Final archive/checksum handoff or publication remains pending explicit approval.
- Exact next step: Hand off `/tmp/cli-ai-agent-starter-kit-codex-0.3.0.tar.gz` and `/tmp/cli-ai-agent-starter-kit-codex-0.3.0.tar.gz.sha256`, or publish them only after explicit approval.

## 2026-06-25T00:04:56Z — Phase E local release handoff

- Objective/phase: Complete Phase E by preparing local 0.3.0 release artifacts for handoff.
- Files/subsystems changed: `docs/PROGRESS.md` and this implementation note. Release artifacts are written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Treated the user's proceed request as approval for local artifact handoff only, not publication. No GitHub release, remote upload, package publication, or deployment is performed. The checksum is intentionally kept in the sidecar file rather than embedded in this note so the release archive can include completed release notes without a self-referential checksum.
- Commands and tests run: Pending final archive rebuild and clean extraction check after this note.
- Results: Phase E is marked complete for local handoff; final artifact paths and checksum will be reported in the final response.
- Security/data/performance/UX implications: Local handoff only. No Codex login, OpenAI request, package installation, `sudo`, GitHub creation, remote push, deployment, or credential inspection is performed.
- Unresolved problems: External acceptance remains pending: live Codex OAuth on CachyOS and a real generated project carried through its first Codex implementation phase.
- Exact next step: Rebuild the 0.3.0 archive with these final notes, regenerate checksum sidecar, and rerun clean extraction verification.

## 2026-06-25T03:10:22Z — External acceptance local dry-run

- Objective/phase: Exercise the local half of external acceptance with a disposable generated project before any real Codex task.
- Files/subsystems changed: `docs/PROGRESS.md` and this implementation note.
- Behavior/design decisions: Generated `/tmp/agent-starter-acceptance-small` from `examples/cli-python-sqlite-codex.json` with `--path` override. This validates the beginner path for a small idea without launching Codex, making an OpenAI request, publishing to GitHub, installing packages, or copying files with rsync.
- Commands and tests run: `./agent-starter generate --answers examples/cli-python-sqlite-codex.json --path /tmp/agent-starter-acceptance-small`; `./agent-starter validate /tmp/agent-starter-acceptance-small`; `./agent-starter status /tmp/agent-starter-acceptance-small`; `./agent-starter github-ready /tmp/agent-starter-acceptance-small`; `./agent-starter prompt /tmp/agent-starter-acceptance-small --template feature --phase "first implementation" --request "Build the smallest useful version of the CLI idea and keep the generated docs current."`; `./agent-starter rsync-plan /tmp/agent-starter-acceptance-small /tmp/agent-starter-acceptance-small-backup`.
- Results: Generation created 44 files, initialized local Git without a commit or remote, and passed starter validation. `status` reported Codex CLI installed and authorized through the CLI status surface, required generated files present, Git on `main` with uncommitted starter files, no origin remote, GitHub Actions deferred, AI-local artifacts ignored, and `NEXT_STEPS.md` as the next action. `github-ready` correctly exited nonzero and recommended against creating a GitHub repository or pushing because the local repo still had uncommitted files, while local checks passed. `prompt` generated a Codex-specific feature continuation prompt that instructs the agent to inspect docs/code, update tests and implementation notes, preserve safety boundaries, and report verification. `rsync-plan` printed a reviewable command and remained plan-only without copying data.
- Security/data/performance/UX implications: This verifies the intended local-first beginner workflow and GitHub pause behavior. No real Codex launch, Codex login, OpenAI request, package installation, `sudo`, GitHub creation, remote push, deployment, real rsync execution, or credential inspection was performed.
- Unresolved problems: Live external acceptance remains pending: a user completes `codex login` with the intended account on CachyOS, launches Codex from a generated project, and carries the first real implementation phase through `docs/11-IMPLEMENTATION-NOTES.md`.
- Follow-up verification: `./scripts/check.sh` passed after this note was added. The check ran Python syntax compilation, 55 unit/integration tests, shell syntax checks, source-tree CLI version check showing `agent-starter 0.3.0`, generation smoke test with 44 generated files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. The 0.3.0 archive was rebuilt with this note, scanned for excluded local/cache paths, extracted to `/tmp/agent-starter-release-check.fTG4Yg/cli-ai-agent-starter-kit-codex-0.3.0`, and its extracted `./scripts/check.sh` also passed with 55 tests and smoke checks. The refreshed checksum sidecar verified with `sha256sum -c`.
- Exact next step: Perform live external acceptance only when a human is ready to run Codex: confirm the intended account with `codex login`, launch Codex from a generated project, complete the first real implementation phase, and inspect the generated `docs/11-IMPLEMENTATION-NOTES.md` handoff.

## 2026-06-25T04:58:15Z — GitHub publication preparation

- Objective/phase: Prepare the source tree for an explicit GitHub repository and release publication request.
- Files/subsystems changed: `.gitignore`, `CHANGELOG.md`, and this implementation note.
- Behavior/design decisions: Added `.agents/` and `.codex/` to the source repository ignore rules before staging the initial release commit. This keeps local AI workspace metadata out of the GitHub repository while preserving the checked release source and generated project templates.
- Commands and tests run: `gh --version`; `gh auth status`; `./scripts/check.sh`; `git init -b main`; `git add .`; `git diff --cached --name-only`; staged-name scan for `.agents/`, `.codex/`, `dist/`, caches, `.pyc`, and env files.
- Results: `gh` is installed, but `gh auth status` reported the stored `omensealed` token is invalid and must be refreshed before remote repository creation can proceed. `./scripts/check.sh` passed with 55 tests and smoke checks after the ignore update. The staged-name scan found no local AI metadata, release artifacts, caches, bytecode, or env files in the index.
- Security/data/performance/UX implications: This is a publication hygiene change only. No GitHub repository, release, remote push, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, or credential inspection was performed.
- Unresolved problems: GitHub publication is blocked until the human refreshes GitHub CLI authentication for `omensealed`.
- Exact next step: After `gh auth refresh -h github.com` succeeds for `omensealed`, commit the staged release source, create `omensealed/omen-agentkit`, push `main`, and publish the `v0.3.0` release with the tarball and checksum.

## 2026-06-25T05:02:38Z — GitHub release publication

- Objective/phase: Publish the approved 0.3.0 release to GitHub.
- Files/subsystems changed: `docs/PROGRESS.md` and this implementation note.
- Behavior/design decisions: Created `omensealed/omen-agentkit` as a private GitHub repository to avoid accidental public publication. Published GitHub release `v0.3.0` with the verified source archive and SHA-256 sidecar. The release tag targets the checked `Release 0.3.0` source commit.
- Commands and tests run: `gh auth refresh -h github.com`; `gh auth status`; `git commit -m "Release 0.3.0"`; `gh repo view omensealed/omen-agentkit`; `gh repo create omensealed/omen-agentkit --private --source . --remote origin --push --description "CLI AI Agent Starter Kit - Codex Edition"`; `gh release create v0.3.0 ...`; `gh release view v0.3.0 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: GitHub CLI authentication for `omensealed` was refreshed through the official device flow. The private repository was created at `https://github.com/omensealed/omen-agentkit`, `main` was pushed, and release `v0.3.0` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.3.0`. Release verification reported both assets uploaded. The tarball asset digest is `sha256:ebc490a87d7c4fde98ca84dfeca551836c40b16814541bef7411b9bb6505b868`.
- Security/data/performance/UX implications: GitHub publication was explicitly requested by the human. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, or credential inspection was performed. GitHub CLI printed only a masked token in status output; no token value was copied or persisted by this project.
- Unresolved problems: Live Codex external acceptance remains pending: a human launches Codex from a generated project and carries the first real implementation phase through `docs/11-IMPLEMENTATION-NOTES.md`.
- Exact next step: Push this documentation-only publication record to `main`.

## 2026-06-27T14:53:50Z — Repo-local Agent Kit Codex skill workflow

- Objective/phase: Add a lean, versioned `$agentkit` Codex skill workflow so short requests inside an already-open Codex session can be expanded into full Agent Kit prompts without turning the starter into an orchestration platform.
- Files/subsystems changed: `agent_starter/idea_prompts.py`, `agent_starter/codex_skill.py`, `agent_starter/cli.py`, `agent_starter/models.py`, `agent_starter/generator.py`, `agent_starter/templates.py`, `agent_starter/wizard.py`, `tests/test_idea_prompts.py`, `tests/test_codex_skill.py`, `tests/test_wizard.py`, examples JSON, `README.md`, `docs/AGENT-INTEGRATION.md`, `docs/SECURITY-MODEL.md`, `docs/DEVELOPMENT.md`, `docs/TEMPLATE-CATALOG.md`, `docs/ANSWER-FILE-REFERENCE.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this note.
- Behavior/design decisions: Added `agent-starter idea-prompt` with `--mode/--idea` and `--from-codex --arguments` parsing for modes `plan`, `implement`, `fix`, `review`, `test`, `refactor`, and `docs`. Prompt files are written under `docs/agent-prompts/` with dated safe slugs and default output prints only the prompt path unless `--print` or `--json` is requested. Added `agent-starter codex skill-status`, `install-agentkit-skill`, `update-agentkit-skill`, and `uninstall-agentkit-skill`. The bundled Agent Kit skill version is `0.1.0`; `SKILL.md` uses only standard `name` and `description` front matter plus concise workflow instructions, while Agent Kit-specific version/install metadata lives in `.agents/skills/agentkit/agentkit-skill.json`. Generated projects default `codex_agentkit_skill` to `true`, with answers-file opt-out preserving backward compatibility for old answers that omit the field. Managed replacements create timestamped backups; non-managed skill content is refused unless forced; normal uninstall removes only Agent Kit-managed files and preserves extra user files.
- Commands and tests run: Baseline `./scripts/check.sh` passed before edits with 55 tests and smoke checks. Read `AGENTS.md`, required maintainer docs, the cached Codex manual Agent Skills section, implementation, tests, examples, and generated-template contracts. Focused feature tests: `python3 -m unittest tests.test_idea_prompts tests.test_codex_skill -v` passed with 14 tests. Broader focused suite: `python3 -m unittest tests.test_cli tests.test_generator tests.test_templates tests.test_wizard tests.test_idea_prompts tests.test_codex_skill -v` passed with 57 tests. Full unit suite: `python3 -m unittest discover -s tests -v` passed with 69 tests. Generated `/tmp/agentkit-skill-inspect.Uzwnya/enabled` from `examples/cli-python-sqlite-codex.json`, inspected `.agents/skills/agentkit/SKILL.md` and `agentkit-skill.json`, ran `agent-starter codex skill-status`, and generated a prompt with `agent-starter idea-prompt --from-codex --arguments "implement Add SQLite save/load support" --json`. Generated `/tmp/agentkit-skill-inspect.Uzwnya/disabled` with `codex_agentkit_skill: false` and confirmed `.agents/skills/agentkit` and generated `$agentkit` notes were absent. `python3 -m compileall -q agent_starter tests starter.py` passed. `bash -n install.sh uninstall.sh scripts/*.sh` passed. `./scripts/check.sh` passed after implementation with 69 tests, shell syntax checks, source-tree CLI version check, generation smoke test creating 46 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Results: New generated projects can include `.agents/skills/agentkit/SKILL.md` and `agentkit-skill.json`; disabled projects omit them. `idea-prompt` preserves the short idea, writes a full mode-aware prompt under `docs/agent-prompts/`, includes implementation-note and safety requirements, and works without Codex installed. Source docs now explain that `$agentkit` is a Codex skill invocation, not a slash command, daemon, MCP server, app-server, SDK path, or background automation.
- Security/data/performance/UX implications: The feature is local file/prompt generation only. It does not run `codex login`, inspect Codex credentials, modify `~/.codex/config.toml`, use permission-bypass flags, call OpenAI or GitHub, install packages, run `sudo`, create remotes, push, publish, deploy, or add third-party runtime dependencies. User-provided idea text is handled through Python arguments and file writes, not `shell=True`.
- Unresolved problems: Live Codex recognition of a freshly installed repo-local skill remains a user acceptance check inside Codex; documentation notes that a Codex restart may be required if the skill does not appear immediately.
- Follow-up verification: `./scripts/check.sh` passed after this note was added. The check ran Python syntax compilation, 69 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test creating 46 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Exact next step: Inspect `git status --short` and prepare a follow-up commit/release decision if desired.

## 2026-06-27T20:16:36Z — Rootless Podman sandbox support

- Objective/phase: Add optional rootless Podman project sandbox support so generated projects can run build/test/toolchain work, and optionally Codex itself, inside project-scoped container boundaries.
- Files/subsystems changed: `agent_starter/models.py`, `agent_starter/wizard.py`, `agent_starter/generator.py`, `agent_starter/templates.py`, `agent_starter/cli.py`, new `agent_starter/sandbox.py`, `agent_starter/idea_prompts.py`, `tests/test_models.py`, `tests/test_wizard.py`, `tests/test_cli.py`, `tests/test_sandbox.py`, `tests/test_idea_prompts.py`, `README.md`, `docs/AGENT-INTEGRATION.md`, `docs/SECURITY-MODEL.md`, `docs/DEVELOPMENT.md`, `docs/TEMPLATE-CATALOG.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this note.
- Behavior/design decisions: Added `SandboxConfig` with `none`, `toolchain`, `codex`, and `files-only` modes while preserving old answers-file compatibility. The wizard asks whether to generate a rootless Podman sandbox, defaults new projects to toolchain mode, defaults existing projects to no sandbox, requires explicit Codex-inside-container selection, and requires explicit autonomous first-run prompt selection. Generated sandbox files include `.agent-starter/sandbox/Containerfile`, `sandbox.json`, `README.md`, `docs/12-SANDBOX.md`, `docs/agent-prompts/create-container-handoff.md`, and `scripts/sandbox/*` helpers. Codex-inside-container mode generates `codex`, `codex-login`, `resume`, and `codex-exec` scripts using a project-specific Codex home volume. MariaDB/PostgreSQL projects get dev-only database scripts; web/API projects get a local web helper; game/Godot projects get headless sandbox checks plus host playtest guidance. `agent-starter sandbox doctor` performs read-only sandbox readiness checks and prints reviewable CachyOS install guidance without running `sudo`. `$agentkit` idea prompts now include sandbox-aware verification guidance when metadata exists.
- Commands and tests run: Fixed the wizard regression from new prompt insertion with `python3 -m unittest tests.test_wizard -v`, passed 6 tests. Focused sandbox/CLI tests: `python3 -m unittest tests.test_sandbox tests.test_cli -v`, passed 30 tests. Focused idea-prompt/sandbox tests: `python3 -m unittest tests.test_idea_prompts tests.test_sandbox -v`, passed 17 tests. Template/generator/sandbox tests: `python3 -m unittest tests.test_sandbox tests.test_templates tests.test_generator -v`, passed 29 tests. Full unit suite: `python3 -m unittest discover -s tests -v`, passed 85 tests. Python syntax: `python3 -m compileall -q agent_starter tests starter.py`, passed. Generated inspection projects under `/tmp/agentkit-sandbox-inspect.DJH7hn`: sandbox disabled, toolchain sandbox, Codex-inside-container sandbox with autonomous prompt, MariaDB sandbox, and Godot/game sandbox. Generated script syntax: `find /tmp/agentkit-sandbox-inspect.DJH7hn -path '*/scripts/*' -type f -print0 | xargs -0 bash -n`, passed. Generated a sandbox-aware prompt with `./agent-starter idea-prompt --project /tmp/agentkit-sandbox-inspect.DJH7hn/mariadb --from-codex --arguments "implement Add SQLite save/load" --json` and verified it references sandbox checks, database helper scripts, and no host credential mounts. Full source check: `./scripts/check.sh`, passed with 85 tests, shell syntax checks, source-tree CLI version check, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Results: Disabled projects omit sandbox files. Toolchain sandbox projects generate core Podman files and scripts. Codex-inside-container projects generate project-scoped Codex home volume launch/login/resume/exec scripts and optional `FIRST_RUN_AUTONOMOUS.md`. MariaDB projects generate `db-up`, `db-down`, `db-shell`, `db-logs`, and `.env.sandbox.example` with dev-only credentials and `127.0.0.1` host binding. Godot/game projects generate `scripts/sandbox/headless-test`, `scripts/playtest-host`, and docs that do not promise GUI/GPU/audio/controller container passthrough. Generated scripts do not mount host `~/.codex`, `~/.ssh`, browser profiles, host home, or use `--dangerously-bypass-approvals-and-sandbox`.
- Security/data/performance/UX implications: This is sandbox file generation and read-only diagnostics only. No Podman build/run, Codex login, OpenAI request, GitHub remote creation, push, deployment, package installation, `sudo`, production data access, rsync execution, credential inspection, host Codex auth copy, or host session import was performed. The generated docs describe rootless Podman as risk reduction, not complete isolation; mounted project files remain writable by container commands.
- Unresolved problems: This first slice does not run a real Podman integration smoke test because normal tests must not require Podman. Containerfile package choices and Codex CLI installation pinning can be tightened later after deciding an official pinned install policy for container images. Advanced GUI/audio/controller passthrough remains intentionally deferred.
- Follow-up verification: `./scripts/check.sh` passed again after this implementation note was appended. The check ran Python syntax compilation, 85 unit/integration tests, shell syntax checks, source-tree CLI version check, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Exact next step: Inspect the current diff and decide whether to commit this sandbox feature slice, then optionally perform a manual Podman smoke test on CachyOS with explicit user approval.

## 2026-06-27T20:28:31Z — Sandbox answer-file documentation follow-up

- Objective/phase: Close the documentation/schema gap after the first sandbox feature commit by documenting the new answer-file fields and updating curated examples.
- Files/subsystems changed: `docs/ANSWER-FILE-REFERENCE.md`, `docs/USER-GUIDE.md`, `docs/ARCHITECTURE.md`, `examples/cli-python-sqlite-codex.json`, `examples/browser-game-codex.json`, `examples/existing-php-mariadb-renovation.json`, and this note.
- Behavior/design decisions: New-project examples now show explicit toolchain sandbox configuration. The existing-project renovation example explicitly disables sandbox generation to match the safer default. The user guide now includes a rootless Podman sandbox workflow section and the answer-file reference lists all `sandbox` fields, accepted modes, and credential/session non-import guarantees.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_models tests.test_generator tests.test_sandbox -v`; `python3 -m unittest tests.test_wizard -v`; `./scripts/check.sh`.
- Results: Focused CLI/model/generator/sandbox suite passed 44 tests. Wizard suite passed 6 tests after realigning answer-list fixtures for the new sandbox prompts. Full source check passed with 85 tests, shell syntax checks, source-tree CLI version check, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Documentation-only and example-data update. No Podman build/run, Codex login, package installation, `sudo`, network call, GitHub action, deployment, rsync, credential inspection, host Codex auth copy, or session import was performed.
- Exact next step: Run the full source check and commit this documentation/example follow-up if clean.

## 2026-06-27T21:02:06Z — 0.4.0 local release prep

- Objective/phase: Prepare the completed `$agentkit` skill workflow and rootless Podman sandbox support as local version `0.4.0`.
- Files/subsystems changed: `VERSION`, `agent_starter/__init__.py`, `agent_starter/models.py`, `pyproject.toml`, `tests/test_cli.py`, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Promoted the unreleased `$agentkit` skill and Podman sandbox entries into `CHANGELOG.md` section `0.4.0 — 2026-06-27`. Updated all package/version fields together so generated `ProjectConfig.kit_version`, CLI `--version`, `VERSION`, and `pyproject.toml` agree. Kept the repo-local Agent Kit skill sidecar minimum version at `0.3.0` because the skill workflow remains compatible with that install/update baseline.
- Commands and tests run: `python3 -m unittest tests.test_release tests.test_cli -v`; `./scripts/check.sh`.
- Results: Release/CLI focused tests passed 23 tests. Full source check passed with 85 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.0`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Local version/changelog preparation only. No archive build, checksum publication, GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, Podman build/run, rsync, or credential inspection was performed.
- Exact next step: Run the full source check once more after this note, then commit the 0.4.0 local release prep if clean.

## 2026-06-27T21:08:13Z — 0.4.0 archive verification

- Objective/phase: Build and verify local 0.4.0 release artifacts without publishing them.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Used `git archive` from `HEAD` with prefix `cli-ai-agent-starter-kit-codex-0.4.0/` so the tarball contains tracked release source only and excludes `.git`, local `.agents`, local `.codex`, ignored `dist/`, caches, generated temp workspaces, and other untracked local state. Generated a SHA-256 sidecar and copied both artifacts to `/tmp` for handoff.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.0/ -o dist/cli-ai-agent-starter-kit-codex-0.4.0.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.0.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.0.tar.gz.sha256`; copied the archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.0.tar.gz.sha256`; extracted the archive to `/tmp/agentkit-0.4.0-extract.JZEHR6`; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 85 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.0`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `32ce035cfdf782dc6ce40bc5f1c220e12be4fd05ac0834d05913d61a2ce91d3e`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, Podman build/run, rsync, or credential inspection was performed.
- Exact next step: Rebuild the 0.4.0 archive and checksum once more with this note included, verify the refreshed checksum and extracted archive, then commit this archive-verification record if clean.

## 2026-06-27T21:12:30Z — GitHub release publication for 0.4.0

- Objective/phase: Publish the verified 0.4.0 release artifacts to the existing private GitHub repository.
- Files/subsystems changed: This implementation note only. The release tag points to commit `885ee04`.
- Behavior/design decisions: Pushed the five local commits from `main` to `origin/main`, created annotated tag `v0.4.0`, pushed that tag, and published GitHub release `v0.4.0` with the verified archive and SHA-256 sidecar. The release assets are the same local files generated from `git archive` after the archive-verification commit.
- Commands and tests run: `git status --short --branch`; `git remote -v`; `git tag --list 'v0.4.0'`; `gh release view v0.4.0 --repo omensealed/omen-agentkit --json tagName,url,isDraft,isPrerelease`; `git push origin main`; `git tag -a v0.4.0 -m "Release 0.4.0"`; `git push origin v0.4.0`; `gh release create v0.4.0 ...`; `gh release view v0.4.0 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: `main` was pushed from `79f9c17` to `885ee04`. Tag `v0.4.0` was created and pushed. GitHub release `v0.4.0` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.0`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:296bf4063b1982ae862bd3c14eb2ba761251fd3acd45f7e311a52003e8d43aaa`; the checksum sidecar asset digest is `sha256:05cff2e15c818ff24bf9ad841ff07fd1b89e2a14d06493ae2e7ffe8987412591`.
- Security/data/performance/UX implications: GitHub publication was performed as the next release action. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-27T21:35:49Z — Podman smoke test and 0.4.1 patch prep

- Objective/phase: Run the manual rootless Podman smoke test for generated sandbox projects and fix the issue it exposed.
- Files/subsystems changed: `agent_starter/sandbox.py`, `tests/test_sandbox.py`, `VERSION`, `agent_starter/__init__.py`, `agent_starter/models.py`, `pyproject.toml`, `tests/test_cli.py`, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: The generated ephemeral toolchain runners no longer pass a fixed `--name agentkit-<project>-dev` to `podman run --rm`. This avoids container-name collisions after parallel or interrupted `scripts/sandbox/exec`, `scripts/sandbox/check`, or `scripts/sandbox/shell` runs. Codex-inside-container scripts keep project-scoped names where a named interactive session is useful.
- Commands and tests run: `command -v podman && podman --version && podman info --format '{{.Host.Security.Rootless}}'`; generated `/tmp/agentkit-podman-smoke.FItK9r/project`; ran generated `scripts/sandbox/doctor`; ran generated `scripts/sandbox/build`; reran generated `scripts/sandbox/doctor`; ran generated `scripts/sandbox/exec python --version`; ran generated `scripts/sandbox/check`; observed the fixed-name collision; patched `agent_starter/sandbox.py`; ran `python3 -m unittest tests.test_sandbox -v`; generated `/tmp/agentkit-podman-smoke-fixed.SVdZTj/project`; ran generated `scripts/sandbox/doctor`, `scripts/sandbox/exec python --version`, and `scripts/sandbox/check`.
- Results: Host Podman was found at `/usr/bin/podman`, version `5.8.3`, and reported rootless mode `true`. Initial generated doctor passed and image build completed successfully, tagging `localhost/agentkit-podman-smoke:dev`. The initial parallel smoke exposed a container-name collision: `agentkit-podman-smoke-dev` was already in use. After removing the fixed ephemeral container name, focused sandbox tests passed 10 tests. The regenerated fixed smoke project reported the sandbox image exists, `scripts/sandbox/exec python --version` printed `Python 3.14.6`, and `scripts/sandbox/check` passed inside the container with the expected placeholder build/lint/test output and documentation invariant check.
- Security/data/performance/UX implications: The smoke test pulled the Arch base image and installed selected packages inside the rootless Podman image only. It did not run `sudo`, install host packages, run Codex login, inspect/copy Codex credentials, mount host `~/.codex`, mount host `~/.ssh`, push/deploy, rsync, or access production data. The generated commands produced non-fatal Podman warnings when run without a TTY.
- Unresolved problems: The noninteractive `podman run -it` warning is noisy but non-fatal; a later polish pass can split interactive and noninteractive sandbox helpers.
- Follow-up verification: `./scripts/check.sh` passed after the 0.4.1 version bump and smoke-test fix. The check ran 86 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.1`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Exact next step: Commit the 0.4.1 patch prep, then build and verify a patch release archive.

## 2026-06-27T21:38:01Z — 0.4.1 archive verification

- Objective/phase: Build and verify local 0.4.1 patch release artifacts after the Podman smoke-test fix.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `009e704` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.1-extract.8hrbD4`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.1/ -o dist/cli-ai-agent-starter-kit-codex-0.4.1.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.1.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.1.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.1.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 86 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.1`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `48ccfcda1a6902a2ebb4b51d2f67d426ec96cc32cbc0ca938a3e73eb5f42917e`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, Podman build/run, rsync, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.1 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-27T21:40:24Z — GitHub release publication for 0.4.1

- Objective/phase: Publish the 0.4.1 patch release that fixes the generated sandbox toolchain container-name collision found by the manual Podman smoke test.
- Files/subsystems changed: This implementation note only. The release tag points to commit `2de6639`.
- Behavior/design decisions: Pushed patch commits to `origin/main`, created annotated tag `v0.4.1`, pushed that tag, and published GitHub release `v0.4.1` with the verified archive and SHA-256 sidecar.
- Commands and tests run: `git status --short --branch`; `git tag --list 'v0.4.1'`; `gh release view v0.4.1 --repo omensealed/omen-agentkit --json tagName,url,isDraft,isPrerelease`; `git push origin main`; `git tag -a v0.4.1 -m "Release 0.4.1"`; `git push origin v0.4.1`; `gh release create v0.4.1 ...`; `gh release view v0.4.1 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: `main` was pushed from `cf10fb5` to `2de6639`. Tag `v0.4.1` was created and pushed. GitHub release `v0.4.1` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.1`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:7ca72c80ea911a3c55cf77057e978995e3055c17c6bccd56d505ba3bb0ae0fa9`; the checksum sidecar asset digest is `sha256:626376441cfdc99c85283b35e3b480b3a5688018e55133902b3be1f6d670c78b`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-27T21:49:06Z — 0.4.2 noninteractive Podman TTY polish

- Objective/phase: Remove the nonfatal Podman `The input device is not a TTY` warning from generated noninteractive sandbox helpers after the 0.4.1 manual smoke test.
- Files/subsystems changed: `agent_starter/sandbox.py`, `tests/test_sandbox.py`, `tests/test_cli.py`, `VERSION`, `agent_starter/__init__.py`, `agent_starter/models.py`, `pyproject.toml`, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Changed generated `run_project_container` to use `podman run --rm` for `scripts/sandbox/exec` and `scripts/sandbox/check`. Added an interactive-only helper in `scripts/sandbox/shell` that still uses `podman run --rm -it`. Kept the 0.4.1 fixed-container-name collision fix intact and bumped package metadata to `0.4.2` because `0.4.1` was already published.
- Commands and tests run: `python3 -m unittest tests.test_sandbox -v`; `python3 -m unittest tests.test_sandbox tests.test_cli tests.test_release -v`; `python3 -m compileall -q agent_starter tests starter.py`; generated disposable sandbox project at `/tmp/agentkit-podman-tty.L7XcG7/project`; ran generated `scripts/sandbox/doctor`, `scripts/sandbox/build`, `scripts/sandbox/exec python --version`, and `scripts/sandbox/check`; `./scripts/check.sh`.
- Results: Focused sandbox tests passed 11 tests. Focused sandbox/CLI/release tests passed 34 tests. Compileall passed. The disposable generated Podman project created 58 files, `scripts/sandbox/exec python --version` printed `Python 3.14.6`, `scripts/sandbox/check` passed, and no `input device is not a TTY` warning appeared in `exec` or `check` output. Full source check passed with 87 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.2`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: The smoke test built and ran the generated rootless Podman sandbox only. It did not run `sudo`, install host packages, run Codex login, inspect/copy Codex credentials, mount host `~/.codex`, mount host `~/.ssh`, push/deploy, rsync, or access production data.
- Exact next step: Commit the 0.4.2 patch prep, then build and verify a patch release archive.

## 2026-06-27T21:51:14Z — 0.4.2 archive verification

- Objective/phase: Build and verify local 0.4.2 patch release artifacts after the noninteractive Podman helper polish.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `deeec59` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.2-extract.hod8VD`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.2/ -o dist/cli-ai-agent-starter-kit-codex-0.4.2.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.2.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.2.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.2.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 87 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.2`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `b32e726fc4aa3e65831d605f28589a8dee7d76dfe0dc4c0e938718682cf12720`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.2 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-27T21:52:30Z — GitHub release publication for 0.4.2

- Objective/phase: Publish the 0.4.2 patch release that removes noninteractive Podman TTY warnings from generated sandbox helpers.
- Files/subsystems changed: This implementation note and `docs/PROGRESS.md` only. The release tag points to commit `bcb982b`.
- Behavior/design decisions: Pushed patch commits to `origin/main`, created annotated tag `v0.4.2`, pushed that tag, and published GitHub release `v0.4.2` with the verified archive and SHA-256 sidecar.
- Commands and tests run: Final archive rebuild from `HEAD`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.2.tar.gz.sha256`; extracted final archive to `/tmp/agentkit-0.4.2-final-extract.BEewMM`; ran extracted `./scripts/check.sh`; `git push origin main`; `git tag -a v0.4.2 -m "Release 0.4.2"`; `git push origin v0.4.2`; `gh release create v0.4.2 ...`; `gh release view v0.4.2 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: Final archive checksum verification passed. Extracted final archive `./scripts/check.sh` passed with 87 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.2`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. `main` was pushed from `8b85a58` to `bcb982b`. Tag `v0.4.2` was created and pushed. GitHub release `v0.4.2` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.2`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:4f85c0967fee46160e6a43fd7a0ceb1bed1bd319ece96d4f2fb021233f9817bf`; the checksum sidecar asset digest is `sha256:352e79795544a3003bc1804811e5e621fe169a4739156c387cfbefa761643ecc`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-27T23:33:00Z — 0.4.3 sandbox contract fallback fix

- Objective/phase: Fix the sandbox contract gap found by the `canary-notes` generated-project test, where sandbox files existed but Codex completed Phase 0 with host scripts after `scripts/sandbox/doctor` failed.
- Files/subsystems changed: `agent_starter/templates.py`, `agent_starter/sandbox.py`, `agent_starter/idea_prompts.py`, sandbox/idea-prompt tests, source docs, version metadata, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Clarified that `toolchain` sandbox mode means host Codex edits the local mounted project files while build/test/toolchain commands run through Podman. Removed the permissive generated first-prompt wording that told Codex not to let sandbox setup block orientation. Generated first prompts, `FIRST_RUN_AUTONOMOUS.md`, `docs/12-SANDBOX.md`, and `$agentkit` idea prompts now say that requested sandbox `doctor`, `build`, or `check` failures must be recorded and stop as `BLOCKED_ENVIRONMENT`, unless the human explicitly approves a temporary host-only fallback.
- Canary evidence: `../omen-agentkit-0.4.2/canary-notes/.agent-starter/project.json` had sandbox enabled in `toolchain` mode and generated sandbox scripts existed, but no `agentkit-canary-notes:dev` Podman image/container existed. Its implementation notes showed `scripts/sandbox/doctor` failed with `Failed to obtain podman configuration: set sticky bit on: chmod /run/user/1000/libpod: read-only file system`, after which Codex continued with host `./scripts/check.sh`. That behavior was too soft for a user-requested sandbox.
- Commands and tests run: `python3 -m unittest tests.test_sandbox tests.test_idea_prompts tests.test_templates -v`; `python3 -m unittest tests.test_sandbox tests.test_idea_prompts tests.test_templates tests.test_release tests.test_cli -v`; `python3 -m compileall -q agent_starter tests starter.py`; generated `/tmp/agentkit-sandbox-contract.ryL6GJ/project` with toolchain sandbox and autonomous prompt enabled; inspected `FIRST_PROMPT.md`, `docs/12-SANDBOX.md`, and `FIRST_RUN_AUTONOMOUS.md`; `./scripts/check.sh`.
- Results: Focused sandbox/idea/template tests passed 30 tests. Focused sandbox/idea/template/release/CLI tests passed 53 tests. Compileall passed. The generated inspection project created 60 files and contained the stricter sandbox contract in first prompt, sandbox docs, and autonomous prompt. Full source check passed with 87 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.3`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: This is prompt/template/documentation behavior only. It does not add host package installs, `sudo`, Codex login, credential inspection, host secret mounts, GitHub remote creation, push, deploy, rsync execution, or alternate-agent support.
- Exact next step: Commit the 0.4.3 patch prep, then build and verify a patch release archive.

## 2026-06-27T23:35:05Z — 0.4.3 archive verification

- Objective/phase: Build and verify local 0.4.3 patch release artifacts after the sandbox fallback contract fix.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `740a68c` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.3-extract.ZXviln`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.3/ -o dist/cli-ai-agent-starter-kit-codex-0.4.3.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.3.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.3.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.3.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 87 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.3`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `e744e66aaa5b70f79d183a76a8851e2bee7dd89d3e65b32a30100967217cf63e`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, Podman build/run, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.3 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-27T23:36:23Z — GitHub release publication for 0.4.3

- Objective/phase: Publish the 0.4.3 patch release that fixes generated sandbox fallback semantics after the canary project showed host-only continuation.
- Files/subsystems changed: This implementation note and `docs/PROGRESS.md` only. The release tag points to commit `9f0e9aa`.
- Behavior/design decisions: Pushed patch commits to `origin/main`, created annotated tag `v0.4.3`, pushed that tag, and published GitHub release `v0.4.3` with the verified archive and SHA-256 sidecar.
- Commands and tests run: Final archive rebuild from `HEAD`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.3.tar.gz.sha256`; extracted final archive to `/tmp/agentkit-0.4.3-final-extract.5xapSt`; ran extracted `./scripts/check.sh`; `git push origin main`; `git tag -a v0.4.3 -m "Release 0.4.3"`; `git push origin v0.4.3`; `gh release create v0.4.3 ...`; `gh release view v0.4.3 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: Final archive checksum verification passed. Extracted final archive `./scripts/check.sh` passed with 87 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.3`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. `main` was pushed from `3dff773` to `9f0e9aa`. Tag `v0.4.3` was created and pushed. GitHub release `v0.4.3` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.3`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:4e4d3e7993eb57f35f9716e2bedc9e07c2fe3f8deaefa9f4d36222c88e853aa3`; the checksum sidecar asset digest is `sha256:5c865d24d840e030caaeac416fd1660900cb4533257a5f5a2af71033c1b3a81c`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-28T01:08:40Z — 0.4.4 nested Podman guard

- Objective/phase: Fix generated sandbox scripts and docs so Codex sessions already running inside the project container do not accidentally run host-side Podman wrapper scripts and trigger nested Podman failures.
- Files/subsystems changed: `agent_starter/sandbox.py`, `agent_starter/templates.py`, `agent_starter/idea_prompts.py`, sandbox/idea-prompt tests, source docs, version metadata, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Generated project containers now receive `AGENTKIT_INSIDE_SANDBOX=1`. Generated sandbox wrappers share an inside-container detection helper that checks the marker, `/run/.containerenv`, and `/.dockerenv`. Inside the container, `scripts/sandbox/check` runs `./scripts/check.sh` directly, `scripts/sandbox/exec` directly executes its command, and `scripts/sandbox/shell` directly opens `/bin/sh`. Host-only wrappers, including build, Codex launch/login/resume/exec, logs/clean, doctor, and database service scripts, refuse inside-container execution with a clear host-side command message. Web and headless-test helpers run direct project commands inside the container. No Podman socket, privileged mode, host secret mount, or Podman-in-Podman path was added.
- Commands and tests run: `python3 -m unittest tests.test_sandbox tests.test_idea_prompts tests.test_release tests.test_cli -v`; `python3 -m compileall -q agent_starter tests starter.py`; generated `/tmp/agentkit-nested-podman.M1pz15/project` with Codex-inside-container, web, MariaDB, and autonomous prompt enabled; ran `find .../scripts -type f -print0 | xargs -0 bash -n`; inspected generated `scripts/sandbox/check`, `scripts/sandbox/codex`, `scripts/sandbox/db-up`, and `docs/12-SANDBOX.md`; `./scripts/check.sh`.
- Results: Focused sandbox/idea/release/CLI tests passed 46 tests. Compileall passed. The generated inspection project created 69 files, all generated scripts passed Bash syntax, project-container launch scripts included `AGENTKIT_INSIDE_SANDBOX=1`, `scripts/sandbox/check` direct-runs `./scripts/check.sh` inside the container, and host-only wrappers contain the refusal message. Full source check passed with 91 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.4`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: This is generated script/template/documentation behavior only. It prevents accidental nested Podman and preserves the existing no-host-secrets policy. It does not add host package installs, `sudo`, Codex login automation, credential inspection, host secret mounts, Podman socket mounts, privileged containers, GitHub remote creation, push, deploy, rsync execution, or alternate-agent support.
- Exact next step: Commit the 0.4.4 patch prep, then build and verify a patch release archive.

## 2026-06-28T01:10:16Z — 0.4.4 archive verification

- Objective/phase: Build and verify local 0.4.4 patch release artifacts after the nested Podman guard fix.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `ecdda3d` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.4-extract.KOKAYP`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.4/ -o dist/cli-ai-agent-starter-kit-codex-0.4.4.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.4.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.4.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.4.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 91 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.4`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `af02b3e00765942c9c7adddb351d7c82a8f3aa768306d731b20b07ddc046656b`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, Podman build/run, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.4 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-28T01:11:53Z — GitHub release publication for 0.4.4

- Objective/phase: Publish the 0.4.4 patch release that prevents accidental nested Podman from generated sandbox wrappers.
- Files/subsystems changed: This implementation note and `docs/PROGRESS.md` only. The release tag points to commit `319f516`.
- Behavior/design decisions: Pushed patch commits to `origin/main`, created annotated tag `v0.4.4`, pushed that tag, and published GitHub release `v0.4.4` with the verified archive and SHA-256 sidecar.
- Commands and tests run: Final archive rebuild from `HEAD`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.4.tar.gz.sha256`; extracted final archive to `/tmp/agentkit-0.4.4-final-extract.DB1lPw`; ran extracted `./scripts/check.sh`; `git push origin main`; `git tag -a v0.4.4 -m "Release 0.4.4"`; `git push origin v0.4.4`; `gh release create v0.4.4 ...`; `gh release view v0.4.4 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: Final archive checksum verification passed. Extracted final archive `./scripts/check.sh` passed with 91 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.4`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. `main` was pushed from `c212baf` to `319f516`. Tag `v0.4.4` was created and pushed. GitHub release `v0.4.4` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.4`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:59005c3636290508d4f6bb6d11c31be57ac9ec580665de202a8ed835792963a9`; the checksum sidecar asset digest is `sha256:4c492ae8ad3e6dc53a2dd4ea80f4b2f5c16a1426f4c0586e609b303fd3a09185`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-28T01:42:05Z — 0.4.5 host Podman preflight contract

- Objective/phase: Fix the remaining sandbox misunderstanding where a host Codex session could interpret rootless Podman bootstrap failure as a need for full Codex/host permissions.
- Files/subsystems changed: `agent_starter/templates.py`, `agent_starter/sandbox.py`, `agent_starter/idea_prompts.py`, sandbox/idea-prompt tests, source docs, version metadata, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Generated prompts now say host-side sandbox preflight (`scripts/sandbox/doctor` and `scripts/sandbox/build`) should be run by the human from a normal host terminal before launching Codex, or by Codex only when the current Codex sandbox/approval policy already permits rootless Podman access. If rootless Podman fails because Codex cannot access runtime paths such as `/run/user/<uid>/libpod`, Codex must record `BLOCKED_ENVIRONMENT` and tell the human to run host preflight or launch Codex inside the built container. Prompts explicitly prohibit requesting Codex `danger-full-access`, host full-access, privileged containers, or Podman socket mounts to make Podman work.
- Commands and tests run: `python3 -m unittest tests.test_sandbox tests.test_idea_prompts tests.test_release tests.test_cli -v`; `python3 -m compileall -q agent_starter tests starter.py`; generated `/tmp/agentkit-host-preflight.EXO87D/project` with Codex-inside-container and autonomous prompt enabled; generated an `$agentkit` idea prompt with `./agent-starter idea-prompt --project ... --mode implement --idea "Add a first feature" --print`; inspected `FIRST_PROMPT.md`, `FIRST_RUN_AUTONOMOUS.md`, and the generated idea prompt; `./scripts/check.sh`.
- Results: Focused sandbox/idea/release/CLI tests passed 46 tests. Compileall passed. The generated inspection project created 64 files and its prompts direct Codex away from full permissions, toward human-run host preflight or Codex-inside-container. Full source check passed with 91 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.5`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: This is generated prompt/documentation behavior only. It reinforces the sandbox-first design and does not add host package installs, `sudo`, Codex login automation, credential inspection, host secret mounts, Podman socket mounts, privileged containers, GitHub remote creation, push, deploy, rsync execution, or alternate-agent support.
- Exact next step: Commit the 0.4.5 patch prep, then build and verify a patch release archive.

## 2026-06-28T01:45:25Z — 0.4.5 archive verification

- Objective/phase: Build and verify local 0.4.5 patch release artifacts after clarifying the safe host Podman preflight contract.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `7546589` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.5-extract.qJyQi5`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.5/ -o dist/cli-ai-agent-starter-kit-codex-0.4.5.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.5.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.5.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.5.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 91 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.5`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `020220315de211cee8007b3e434faed0fa25c03a058e22ef8ce6e2312bb81574`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, Podman build/run, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.5 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-28T01:46:36Z — GitHub release publication for 0.4.5

- Objective/phase: Publish the 0.4.5 patch release that prevents generated sandbox guidance from treating host rootless Podman runtime restrictions as a reason to ask for Codex full permissions.
- Files/subsystems changed: This implementation note and `docs/PROGRESS.md` only. The release tag points to commit `68e3fce`.
- Behavior/design decisions: Rebuilt the final release archive from `HEAD`, verified the checksum, extracted the archive into `/tmp/agentkit-0.4.5-final-extract.TTrEd9`, ran the extracted full check, pushed `main`, created annotated tag `v0.4.5`, pushed that tag, and published GitHub release `v0.4.5` with the verified archive and SHA-256 sidecar.
- Commands and tests run: Final archive rebuild from `HEAD`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.5.tar.gz.sha256`; extracted final archive to `/tmp/agentkit-0.4.5-final-extract.TTrEd9`; ran extracted `./scripts/check.sh`; `git push origin main`; `git tag -a v0.4.5 -m "Release 0.4.5"`; `git push origin v0.4.5`; `gh release create v0.4.5 ...`; `gh release view v0.4.5 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: Final archive checksum verification passed. Extracted final archive `./scripts/check.sh` passed with 91 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.5`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. `main` was pushed from `9d59411` to `68e3fce`. Tag `v0.4.5` was created and pushed. GitHub release `v0.4.5` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.5`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:e7fdc681bfd91c86772e466ae1af6ad42cf64adeb87b357b333a4171181c1bdf`; the checksum sidecar asset digest is `sha256:b11dbc4d61957d78e7d2087a4e6c01c913e718bf9a7cb47d38f0a8f052740f50`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-28T01:57:34Z — 0.4.6 launch-time sandbox preflight

- Objective/phase: Make sandbox preflight an Agent Kit-managed setup gate before Codex launch, so autonomous first-run workflows do not require Codex full permissions to bootstrap rootless Podman.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/templates.py`, CLI/template tests, source docs, version metadata, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Added `agent-starter sandbox preflight`, which runs generated host-side `scripts/sandbox/doctor`, `scripts/sandbox/build`, and `scripts/sandbox/check` in order from the project root. `agent-starter launch` now runs that preflight automatically for active `toolchain` and `codex` sandbox modes before Codex starts, with `--skip-sandbox-preflight` available only as an explicit bypass. Generated `START_AGENT.sh` also runs the preflight before launching Codex. For Codex-inside-container autonomous kickoff, `agent-starter launch --kickoff` uses `scripts/sandbox/codex-exec` after preflight instead of host Codex, and generated `START_AGENT.sh --kickoff` does the same. The Codex-inside-container launch script path does not include host `codex login` checks.
- Commands and tests run: `python3 -m unittest tests.test_cli tests.test_sandbox tests.test_templates -v`; generated `/tmp/agentkit-preflight.Mh7kYT/project` with Codex-inside-container sandbox and autonomous prompt enabled; inspected generated `START_AGENT.sh`, `NEXT_STEPS.md`, `README.md`, and `FIRST_PROMPT.md`; ran generated script syntax checks with `find .../scripts -type f -print0 | xargs -0 bash -n` and `bash -n START_AGENT.sh`; `./scripts/check.sh`.
- Results: Focused CLI/sandbox/template tests passed 52 tests. Generated inspection project created 63 files; `START_AGENT.sh` runs `agent-starter sandbox preflight "$ROOT"` before `scripts/sandbox/codex-exec` or `scripts/sandbox/codex`, contains no host `codex login status` branch in Codex-inside-container mode, and passed Bash syntax. Full source check passed with 95 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.6`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: This adds a host-side Podman build/check preflight when launching sandbox-enabled projects. It does not run `sudo`, install host packages, mount host Codex credentials, mount SSH keys, inspect credentials, use privileged containers, mount a Podman socket, create GitHub remotes, push, deploy, rsync, or add alternate-agent support. Rootless Podman image builds can still require network access for base images when the user runs a sandbox launch/preflight.
- Exact next step: Commit the 0.4.6 patch prep, then build and verify a patch release archive.

## 2026-06-28T01:59:02Z — 0.4.6 archive verification

- Objective/phase: Build and verify local 0.4.6 patch release artifacts after adding launch-time sandbox preflight.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `941de6b` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.6-extract.Wzfo8E`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.6/ -o dist/cli-ai-agent-starter-kit-codex-0.4.6.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.6.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.6.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.6.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 95 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.6`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `4a8c000c546d57ad785a5325340e1210588f5c12981985eebf1ce019c0d3a210`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, Podman build/run outside generated script verification, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.6 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-28T02:00:28Z — GitHub release publication for 0.4.6

- Objective/phase: Publish the 0.4.6 patch release that makes sandbox preflight an Agent Kit-managed setup gate before Codex launch.
- Files/subsystems changed: This implementation note and `docs/PROGRESS.md` only. The release tag points to commit `4420bc8`.
- Behavior/design decisions: Rebuilt the final release archive from `HEAD`, verified the checksum, extracted the archive into `/tmp/agentkit-0.4.6-final-extract.b4RBpW`, ran the extracted full check, pushed `main`, created annotated tag `v0.4.6`, pushed that tag, and published GitHub release `v0.4.6` with the verified archive and SHA-256 sidecar.
- Commands and tests run: Final archive rebuild from `HEAD`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.6.tar.gz.sha256`; extracted final archive to `/tmp/agentkit-0.4.6-final-extract.b4RBpW`; ran extracted `./scripts/check.sh`; `git push origin main`; `git tag -a v0.4.6 -m "Release 0.4.6"`; `git push origin v0.4.6`; `gh release create v0.4.6 ...`; `gh release view v0.4.6 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: Final archive checksum verification passed. Extracted final archive `./scripts/check.sh` passed with 95 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.6`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. `main` was pushed from `3cd8f9d` to `4420bc8`. Tag `v0.4.6` was created and pushed. GitHub release `v0.4.6` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.6`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:c00f8451bb801a904c950cbf794ea990d277526de527072f7ac8a4027d4244e5`; the checksum sidecar asset digest is `sha256:3e4c6600466a8a49a5f0374b07eab6eb6f41d3cbfb58068e8e01a397b02e31e0`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-28T02:54:19Z — 0.4.7 sandbox preflight stamp and constrained-Codex guidance

- Objective/phase: Fix the remaining canary failure where an already-open host Codex session reran host-side Podman wrappers from inside Codex's constrained sandbox even though pre-Codex setup should have been handled by Agent Kit.
- Files/subsystems changed: `agent_starter/cli.py`, `agent_starter/templates.py`, `agent_starter/sandbox.py`, `agent_starter/idea_prompts.py`, CLI/sandbox/idea-prompt tests, source docs, version metadata, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Canary evidence: In `../omen-agentkit-0.4.6/canary-notes`, the generated project was `toolchain` sandbox mode. Its Codex session reported `scripts/sandbox/doctor` failed with `Failed to obtain podman configuration: set sticky bit on: chmod /run/user/1000/libpod: read-only file system`. From a normal host shell in the same project, `/run/user/1000` was writable, `podman info --format '{{.Host.Security.Rootless}}'` returned `true`, and `scripts/sandbox/doctor` passed with rootless mode, subuid/subgid, sandbox config, and sandbox image all OK. That shows the host setup was usable, while the already-open Codex execution environment could not access the rootless Podman runtime path.
- Behavior/design decisions: `agent-starter sandbox preflight` now writes `.agent-starter/sandbox/preflight.json` after successful host-side doctor/build/check. Generated `FIRST_PROMPT.md`, `FIRST_RUN_AUTONOMOUS.md`, `docs/12-SANDBOX.md`, and `$agentkit` idea prompts now say that if the preflight stamp reports `"status": "passed"`, an already-open constrained Codex session should not rerun `scripts/sandbox/doctor` or `scripts/sandbox/build`. `scripts/sandbox/check` is now recommended from Codex only when the current Codex sandbox/approval policy permits rootless Podman access; otherwise Codex should stop with `BLOCKED_ENVIRONMENT` and ask the human to run verification from a normal host terminal or use Codex-inside-container mode.
- Commands and tests run: `scripts/sandbox/doctor` in `../omen-agentkit-0.4.6/canary-notes`; `podman info --format '{{.Host.Security.Rootless}}'`; `python3 -m unittest tests.test_cli tests.test_sandbox tests.test_templates tests.test_idea_prompts tests.test_release -v`; generated `/tmp/agentkit-preflight-stamp.vwL1P8/project` with toolchain sandbox and autonomous prompt enabled; inspected generated `FIRST_PROMPT.md`, `FIRST_RUN_AUTONOMOUS.md`, `docs/12-SANDBOX.md`, and `README.md`; ran generated script Bash syntax checks; `./scripts/check.sh`.
- Results: Host canary `scripts/sandbox/doctor` passed from the normal shell, confirming the failure is constrained-Codex runtime access rather than broken host Podman. Focused CLI/sandbox/template/idea/release tests passed 61 tests. Generated inspection project created the corrected preflight stamp guidance and all generated scripts passed Bash syntax. Full source check passed with 95 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.7`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: The new preflight stamp is non-secret local metadata under `.agent-starter/sandbox/`. This does not add `sudo`, package installation, credential inspection, host Codex auth mounts, SSH mounts, privileged containers, Podman socket mounts, GitHub remotes, pushes, deploys, rsync execution, or alternate-agent support. It reduces false `BLOCKED_ENVIRONMENT` outcomes caused by asking constrained Codex to redo host preflight.
- Exact next step: Commit the 0.4.7 patch prep, then build and verify a patch release archive.

## 2026-06-28T02:55:32Z — 0.4.7 archive verification

- Objective/phase: Build and verify local 0.4.7 patch release artifacts after adding the sandbox preflight stamp and constrained-Codex guidance.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/` and mirrored under `/tmp`.
- Behavior/design decisions: Built the release archive from commit `7ece8ad` with `git archive`, generated a SHA-256 sidecar, copied both artifacts to `/tmp`, extracted the archive into `/tmp/agentkit-0.4.7-extract.Q7skjj`, and ran the extracted source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.7/ -o dist/cli-ai-agent-starter-kit-codex-0.4.7.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.7.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.7.tar.gz.sha256`; copied archive and checksum to `/tmp`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.7.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Extracted archive `./scripts/check.sh` passed with 95 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.7`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Initial archive checksum before adding this note was `66a3ca5a45119f4fdf9f51854ab9e68b93b7041e9a8238b6f607cf356469e268`; the final archive must be rebuilt after this note so the checksum sidecar reflects the exact final source.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, Podman build/run outside generated script verification, or credential inspection was performed in this archive-verification step.
- Exact next step: Commit this archive-verification record, rebuild the 0.4.7 archive from that commit, rerun extracted archive verification, then tag and publish if clean.

## 2026-06-28T02:56:52Z — GitHub release publication for 0.4.7

- Objective/phase: Publish the 0.4.7 patch release that records successful sandbox preflight and prevents constrained Codex sessions from rerunning host-side Podman bootstrap.
- Files/subsystems changed: This implementation note and `docs/PROGRESS.md` only. The release tag points to commit `8148e65`.
- Behavior/design decisions: Rebuilt the final release archive from `HEAD`, verified the checksum, extracted the archive into `/tmp/agentkit-0.4.7-final-extract.jrDrvJ`, ran the extracted full check, pushed `main`, created annotated tag `v0.4.7`, pushed that tag, and published GitHub release `v0.4.7` with the verified archive and SHA-256 sidecar.
- Commands and tests run: Final archive rebuild from `HEAD`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.7.tar.gz.sha256`; extracted final archive to `/tmp/agentkit-0.4.7-final-extract.jrDrvJ`; ran extracted `./scripts/check.sh`; `git push origin main`; `git tag -a v0.4.7 -m "Release 0.4.7"`; `git push origin v0.4.7`; `gh release create v0.4.7 ...`; `gh release view v0.4.7 --repo omensealed/omen-agentkit --json tagName,name,url,isDraft,isPrerelease,assets`.
- Results: Final archive checksum verification passed. Extracted final archive `./scripts/check.sh` passed with 95 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.7`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. `main` was pushed from `d88801d` to `8148e65`. Tag `v0.4.7` was created and pushed. GitHub release `v0.4.7` was published at `https://github.com/omensealed/omen-agentkit/releases/tag/v0.4.7`. GitHub reports both assets uploaded. The tarball asset digest is `sha256:2901e231d345697ebf44bc13617f794bb0aaf07c345609d83a72756836acd204`; the checksum sidecar asset digest is `sha256:2a3f87a1a34cc0d13265d1006e65afe3c2cc74aac02ea6b565fdbce9e1c6391b`.
- Security/data/performance/UX implications: GitHub publication was performed for the patch release. No package-manager installation, `sudo`, deployment, package registry publication, Codex login, OpenAI request, Podman build/run, rsync, or credential inspection was performed in this publication step.
- Exact next step: Run `./scripts/check.sh` after this publication record, commit the documentation-only record, and push `main`.

## 2026-06-28T03:47:00Z — Optional GUI frontend and generation idempotency fix

- Objective/phase: Start the beginner GUI phase without replacing the CLI, and first fix the repeated-generation idempotency regression affecting timestamp-managed files.
- Files/subsystems changed: `agent_starter/generator.py`, `agent_starter/cli.py`, new `agent_starter/ui_schema.py`, new `agent_starter/gui/*`, `pyproject.toml`, `tests/test_generator.py`, new `tests/test_gui.py`, `README.md`, `docs/DEVELOPMENT.md`, `docs/SECURITY-MODEL.md`, `docs/TEMPLATE-CATALOG.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this note.
- Behavior/design decisions: `generate_project()` now reuses existing generated `created_at`/`updated_at` metadata when a fresh config regenerates the same workspace, and generated Agent Kit skill sidecar timestamps are derived from the project config. This prevents timestamp-only conflicts for `.agent-starter/manifest.json`, `.agent-starter/project.json`, docs memory files, and `.agents/skills/agentkit/agentkit-skill.json` while preserving normal conflict proposals for real content changes. Added optional `agent-starter gui` and `agent-starter-gui`, with `pywebview` as an optional extra only. The GUI uses local static HTML/CSS/JS, builds `ProjectConfig` through `agent_starter.ui_schema`, calls the existing generator/validator, and exposes Codex status/launch through the existing adapter boundary. `pywebview` is imported lazily only when launching the GUI, with a WebKitGTK renderer environment default for better local desktop compatibility.
- Commands and tests run: Verified `../omen-agentkit-0.4.7/canary-notes` with `./scripts/check.sh`; it passed lint/static checks, build syntax checks, 5 Node tests, and documentation invariants. Focused idempotency tests: `python3 -m unittest tests.test_generator.GeneratorTests.test_fresh_config_regeneration_is_idempotent tests.test_generator.GeneratorTests.test_unchanged_generation_is_idempotent -v` passed. Focused GUI tests: `python3 -m unittest tests.test_gui -v` passed 7 tests. Full unit suite: `python3 -m unittest discover -s tests -v` passed 103 tests. Syntax checks: `python3 -m compileall -q agent_starter tests starter.py` passed; `bash -n install.sh uninstall.sh scripts/*.sh` passed. Full source check `./scripts/check.sh` passed with 103 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.7`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. A direct local `agent-starter gui` launch attempt started pywebview on this host but was interrupted with Ctrl-C after GTK/GStreamer desktop-stack warnings, so automated verification remains import/bridge/static smoke rather than display-server acceptance.
- Results: The canary project built from 0.4.7 looks healthy. The GUI bridge can generate and validate a temporary workspace without a display server, and tests confirm Codex status does not start login. The optional GUI packaging metadata includes the `gui` extra, local static assets, and `agent-starter-gui` script entry point. Regeneration with a fresh config no longer creates timestamp-only conflicts for the manifest or Agent Kit skill sidecar.
- Security/data/performance/UX implications: Default CLI installation remains dependency-free at runtime. No React/Vue/Electron/Tauri/Flet/PySide6, CDN assets, daemon, MCP, app server, SDK, plugin automation, alternate-agent support, Codex credential inspection, package installation, `sudo`, GitHub remote creation, push, deploy, rsync execution, OpenAI request, or Codex login was added. GUI payloads reject common credential-like strings and preserve the existing generator safety model.
- Unresolved problems: A real desktop-window acceptance pass still needs to be performed on the target CachyOS desktop stack with `pywebview` installed; local CLI and bridge behavior are verified, but this session did not complete a visual/manual GUI run.
- Exact next step: Run the full source check after this note, then decide whether to polish the GUI flow further or cut the next release candidate.

## 2026-06-28T04:12:00Z — GUI launch polish and sandbox cleanup controls

- Objective/phase: Address first GUI usability feedback and reduce Podman disk churn from generated sandbox projects.
- Files/subsystems changed: `agent_starter/gui/static/app.js`, `agent_starter/gui/static/app.css`, `agent_starter/gui/bridge.py`, `agent_starter/sandbox.py`, `agent_starter/cli.py`, `tests/test_gui.py`, `tests/test_sandbox.py`, `tests/test_cli.py`, `README.md`, `docs/DEVELOPMENT.md`, `docs/TEMPLATE-CATALOG.md`, `docs/PROGRESS.md`, `CHANGELOG.md`, and this note.
- Behavior/design decisions: The GUI launch-Codex action now asks the bridge to close pywebview windows before calling the existing `launch_agent()` path, and preview `<pre>` panes wrap long output/path strings. Generated sandbox `build` now reuses the existing project image by default and requires `scripts/sandbox/build --rebuild` for an explicit rebuild. Generated Podman runs now label project containers/images with `agentkit.project=<project-id>`. Generated `scripts/sandbox/clean` removes labeled project containers, optionally removes the project image with `--image`, and refuses volume deletion unless `--volumes --yes` or `--all --yes` is supplied because volumes may contain Codex container auth/session state, caches, or dev database data. Added `agent-starter sandbox clean` as a source CLI delegate to the generated cleanup script.
- Commands and tests run: Focused test run `python3 -m unittest tests.test_gui tests.test_sandbox.SandboxGenerationTests.test_build_reuses_existing_image_by_default tests.test_sandbox.SandboxGenerationTests.test_clean_script_can_remove_image_and_requires_yes_for_volumes tests.test_sandbox.SandboxGenerationTests.test_sandbox_shell_remains_interactive_without_fixed_name tests.test_cli.CliTests.test_sandbox_clean_delegates_to_generated_script -v` initially exposed a Python f-string/template syntax error in `agent_starter/sandbox.py`; after switching that shell template to placeholder replacement, the same focused run passed 12 tests. Broader focused run `python3 -m unittest tests.test_cli tests.test_sandbox tests.test_gui -v` passed 51 tests. `python3 -m compileall -q agent_starter tests starter.py` passed.
- Results: GUI text preview wrapping and close-before-Codex handoff are covered by tests. Generated sandbox scripts are syntax-valid under compile/generation tests, reuse project images by default, and provide an explicit cleanup path for containers, images, and volumes. No live Podman cleanup was executed during tests.
- Security/data/performance/UX implications: Cleanup remains explicit and project-scoped. Volume deletion requires confirmation because Codex-inside-container auth/session state and development database data can live there. No `sudo`, package installation, Codex login, OpenAI request, GitHub remote creation, push, deploy, rsync execution, credential inspection, host secret mount, Podman socket mount, privileged container, or alternate-agent support was added.
- Unresolved problems: Godot sandbox support still needs a dedicated design pass for headless visual smoke tests and screenshot/artifact capture. The default should stay headless checks plus host playtesting; GUI/GPU/audio/controller passthrough should remain an advanced opt-in profile rather than the default sandbox.
- Exact next step: Run `./scripts/check.sh` after this note.

## 2026-06-28T04:31:00Z — Game/Godot GUI passthrough opt-in

- Objective/phase: Make the sandbox guidance honest for interactive game/Godot workflows that need GPU/audio/controller passthrough, while keeping that extra host exposure explicitly opt-in.
- Files/subsystems changed: `agent_starter/models.py`, `agent_starter/wizard.py`, `agent_starter/ui_schema.py`, GUI static HTML, `agent_starter/sandbox.py`, answer examples, model/wizard/sandbox/gui tests, README, answer-file reference, security model, template catalog, changelog, progress, and this note.
- Behavior/design decisions: Added `SandboxConfig.gui_passthrough`, defaulting to false and loading missing old answer fields safely. The wizard asks about passthrough only after stack selection shows a game/Godot project, so manual selection and AI/local fallback recommendations can both trigger the warning. The GUI exposes the same setting as an advanced checkbox with warning text. Generated game/Godot projects keep `scripts/sandbox/headless-test` and `scripts/playtest-host` by default; `scripts/sandbox/playtest-gui` is generated only when `sandbox.gui_passthrough` is true. That script is host-side only and checks for Wayland, `/dev/dri`, PipeWire, and `/dev/input`, then launches Godot with selected host display/audio/input interfaces exposed to the project container. It does not mount host home, Codex credentials, SSH keys, browser profiles, or the Podman socket.
- Commands and tests run: Focused run `python3 -m unittest tests.test_models tests.test_wizard tests.test_sandbox.SandboxGenerationTests.test_game_projects_generate_headless_and_host_playtest_guidance tests.test_sandbox.SandboxGenerationTests.test_game_gui_passthrough_is_explicit_opt_in tests.test_gui -v` passed 21 tests after correcting the new wizard test answer alignment. Broader focused run `python3 -m unittest tests.test_cli tests.test_sandbox tests.test_models tests.test_wizard tests.test_gui -v` passed 63 tests. Generated a temporary Godot game project with `SandboxConfig(enabled=True, mode="toolchain", gui_passthrough=True)` and confirmed validation passed and `scripts/sandbox/playtest-gui` was created. `python3 -m compileall -q agent_starter tests starter.py` passed.
- Results: Game/Godot projects now warn that interactive container playtesting probably needs explicit passthrough. The generated default remains safer headless checks plus host playtesting; advanced passthrough is visible, documented, and test-covered.
- Security/data/performance/UX implications: Passthrough intentionally exposes selected host GUI/audio/input interfaces and is therefore off by default. No package installation, `sudo`, Codex login, OpenAI request, GitHub remote creation, push, deploy, rsync execution, credential inspection, host home mount, host Codex auth mount, SSH mount, Podman socket mount, privileged container, or alternate-agent support was added.
- Unresolved problems: The advanced passthrough script has not been live-tested against an actual Godot project and Wayland/PipeWire/controller setup; headless visual smoke tests and screenshot/artifact capture remain the next Godot-specific improvement.
- Exact next step: Run `./scripts/check.sh` after this note.

## 2026-06-28T17:21:34Z — Local generated-project ignore and verification

- Objective/phase: Keep local generated test projects out of source control and record the full verification result after the GUI and sandbox cleanup/passthrough phase.
- Files/subsystems changed: `.gitignore` and this note.
- Behavior/design decisions: Added explicit ignores for maintainer-created generated projects `canary-notes2/` and `tic-tac-toe/`. These are local test workspaces, not source files for Agent Kit, and should not be tracked or included in release archives.
- Commands and tests run: `git status --short`; `./scripts/check.sh`.
- Results: Before the ignore update, `canary-notes2/` and `tic-tac-toe/` appeared as untracked directories. After the ignore update, the full source check passed with 109 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.7`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: This is source repository hygiene only. No generated test workspace contents were read into the product, published, deleted, or moved. No Codex login, OpenAI request, GitHub push, package installation, `sudo`, Podman build/run, deployment, rsync, credential inspection, host secret mount, or alternate-agent support was introduced.
- Exact next step: Recheck Git status, then either package the current GUI/sandbox work as the next release candidate or continue with the Godot headless visual smoke/screenshot follow-up.

## 2026-06-28T19:58:19Z — Rootless Podman keep-id ownership hardening

- Objective/phase: Harden generated rootless Podman wrappers for CachyOS/Arch-style hosts so container-created `/workspace` files remain owned by the real host user without assuming UID/GID `1000`.
- Files/subsystems changed: `agent_starter/sandbox.py`, sandbox tests, README, user guide, security model, development guide, template catalog, changelog, progress, and this note.
- Behavior/design decisions: Generated project container wrappers now derive `HOST_UID=$(id -u)` and `HOST_GID=$(id -g)`, pass `--userns=keep-id`, and run as `--user "$HOST_UID:$HOST_GID"`. Toolchain helpers use a container-local home at `/tmp/agentkit-home` and a project cache volume mounted there. Codex-inside-container helpers keep the project-specific Codex home volume at `/home/codex`, set `HOME=/home/codex`, and mount that named volume with `:Z,U`. The generated `Containerfile` no longer creates a hard-coded UID 1000 `codex` user. Generated sandbox metadata records `user_namespace: keep-id`, generated docs explain the ownership model, and both generated/source sandbox doctors report whether Podman exposes `--userns` support.
- Commands and tests run: `python3 -m unittest tests.test_sandbox -v`; generated `/tmp/tmp.4RHxcVGmji/project` with Codex-inside-container, PostgreSQL, and web sandbox settings; ran `find /tmp/tmp.4RHxcVGmji/project/scripts -type f -print0 | xargs -0 bash -n`; inspected generated sandbox metadata/scripts/docs with `rg`; `python3 -m unittest tests.test_cli tests.test_sandbox tests.test_templates tests.test_models tests.test_wizard tests.test_gui -v`; `./scripts/check.sh`.
- Results: Sandbox-focused tests passed 19 tests. The generated inspection project reported `generate_ok=True`, `validate_ok=True`, created 68 files, and all generated scripts passed Bash syntax. Inspection confirmed `--userns=keep-id`, runtime UID/GID variables, `user_namespace: keep-id`, and no `useradd -m -u 1000` in the generated sandbox files. Broader focused CLI/sandbox/template/model/wizard/GUI tests passed 76 tests. Full source check passed with 110 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.7`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: This keeps the default model as host Codex plus containerized toolchain/check commands, while reducing beginner-facing file ownership problems. It does not mount host home, host `~/.codex`, SSH keys, browser profiles, Podman sockets, or production secrets. It does not add privileged containers, nested Podman, package installation, `sudo`, Codex login automation, OpenAI requests, GitHub remotes, pushes, deployments, rsync execution, credential inspection, or alternate-agent support.
- Exact next step: Run the full `./scripts/check.sh` after this note, then decide whether to release the current unreleased GUI/sandbox hardening set or continue with live Godot passthrough/headless screenshot testing.

## 2026-06-28T20:03:23Z — 0.4.8 release prep version bump

- Objective/phase: Prepare the accumulated optional GUI, sandbox cleanup, game passthrough, and rootless Podman keep-id hardening work as release `0.4.8`.
- Files/subsystems changed: `VERSION`, `agent_starter/__init__.py`, `agent_starter/models.py`, `pyproject.toml`, `tests/test_cli.py`, `CHANGELOG.md`, `docs/PROGRESS.md`, and this note.
- Behavior/design decisions: Promoted the current Unreleased changelog entries into `0.4.8 — 2026-06-28`. Updated all package/version fields together so generated `ProjectConfig.kit_version`, CLI `--version`, `VERSION`, and `pyproject.toml` agree.
- Commands and tests run: `python3 -m unittest tests.test_release tests.test_cli.CliTests.test_version_and_toolchains -v`; `./scripts/check.sh`.
- Results: Release consistency and CLI version tests passed 2 tests. Full source check passed with 110 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.8`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test.
- Security/data/performance/UX implications: Version metadata only. No Codex login, OpenAI request, GitHub push, package installation, `sudo`, Podman build/run, deployment, rsync, credential inspection, host secret mount, or alternate-agent support was introduced.
- Exact next step: Run `./scripts/check.sh`, then build and verify the `0.4.8` archive locally if clean.

## 2026-06-28T20:05:56Z — 0.4.8 archive verification

- Objective/phase: Build and verify local `0.4.8` release artifacts after the GUI and sandbox hardening release-prep commit.
- Files/subsystems changed: `docs/PROGRESS.md` and this note. Release artifacts were written under ignored `dist/`.
- Behavior/design decisions: Built `dist/cli-ai-agent-starter-kit-codex-0.4.8.tar.gz` from commit `5895600` with `git archive`, generated `dist/cli-ai-agent-starter-kit-codex-0.4.8.tar.gz.sha256`, verified the checksum, extracted the archive into `/tmp/agentkit-0.4.8-extract.ineUmw`, and ran the extracted full source check.
- Commands and tests run: `git archive --format=tar.gz --prefix=cli-ai-agent-starter-kit-codex-0.4.8/ -o dist/cli-ai-agent-starter-kit-codex-0.4.8.tar.gz HEAD`; `sha256sum dist/cli-ai-agent-starter-kit-codex-0.4.8.tar.gz > dist/cli-ai-agent-starter-kit-codex-0.4.8.tar.gz.sha256`; `sha256sum -c dist/cli-ai-agent-starter-kit-codex-0.4.8.tar.gz.sha256`; extracted the archive; ran extracted `./scripts/check.sh`.
- Results: Checksum verification passed. Initial archive checksum was `a64a787ad0b8b023dd1553721b7a3b85148ad39be5802064110ec792056010b6`. Extracted archive `./scripts/check.sh` passed with 110 tests, shell syntax checks, source-tree CLI version check reporting `agent-starter 0.4.8`, generation smoke test creating 58 files, generated project `scripts/check.sh`, and user-local install/uninstall smoke test. Because this note changes the source after that archive, the final archive must be rebuilt from the commit containing this note before publication.
- Security/data/performance/UX implications: Local archive handoff only. No GitHub push, GitHub release, package publication, deployment, Codex login, OpenAI request, package installation, `sudo`, rsync, Podman build/run, or credential inspection was performed.
- Exact next step: Commit this archive-verification record, rebuild the final `0.4.8` archive from that commit, rerun checksum and extracted archive verification, then tag/publish only if release publication is approved.
